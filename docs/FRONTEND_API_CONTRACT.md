...
| Method & Path | Purpose | Auth | Success `data` | Errors |
| :--- | :--- | :--- | :--- | :--- |
| `POST /api/exams` | Tạo phiên chấm (`classId`, `subject`, `scoreType`, `maxScore` 1–10, `semester`, `academicYear?`, `examType?`; MC bắt buộc `questionCount` 1–50 + `answerKey` JSON **đủ chính xác câu 1..questionCount**, mỗi value A/B/C/D; `idempotencyKey?`) | admin, chunhiem, phuta (lớp mình) + `checkUserClassAccess` | `ExamSession` (`EXS-`, `status:'draft'`); retry cùng `idempotencyKey` trả session đã tạo | 403 ngoài lớp, 404 lớp không tồn tại, 400 thiếu/không đủ answerKey hoặc questionCount không hợp lệ |
...
| `POST /api/exams/:id/results` | Upsert kết quả (`results[]` — `studentId`, `score` 0-`maxScore`, `source` `qr_scan/omr/quick_entry`, `answers?` JSON map câu → A/B/C/D/null, `scanMetadata?` JSON ≤10KB không chứa ảnh). Với MC + `omr/qr_scan`, server parse answers và **tự tính lại** `correct/questionCount × maxScore`; metadata khác `accepted` bị chặn. **phuta check class access ngay tại đây** | admin / chunhiem / phuta | `{ session, saved, upserted, total, adjustments: [{ studentId, clientScore, serverScore }] }` | 403 ngoài lớp, 400 điểm/answers/metadata/thiếu nhi không hợp lệ, 409 phiên đã hoàn tất |
...
| `POST /api/exams/barcode/decode` | Decode code text: form protocol v2 `T2:{sessionHex8}:{studentHex8}:{I|F}:{questionCount}:{checksum4}`, compact v1 `TE:*` hoặc legacy `tntt-exam:*`. V2 validate checksum + count 1–50 | auth | `{ sessionId, studentId, classId, subject, examType, maxScore, questionCount, protocolVersion?, templateMode?, formChecksum? }` | 400 format/checksum không hợp lệ, 403 ngoài lớp, 404 phiên không tồn tại |
...
| **Code Scan Pipeline** | `qr.ts` + `cameraFrame.ts` + `cameraStillCapture.ts` + `examCodeScanner.ts` + `examScanIdentity.ts` + `omrScanConsensus.ts` + `scanQuality.ts` + `scanDiagnostics.ts` + `ExamScanModal.tsx` | Phiếu production mới dùng T2 Alphanumeric (25 module) mang template/số câu/checksum; TE (21 module) và legacy vẫn parse. V2 tự chọn geometry và chặn mismatch số câu. Fixed identity chụp frame sensor tối đa 2200px; auto scan vẫn cần 2 fingerprint liên tiếp. `review_required` (multi/weak mark) khóa Save tới khi sửa. Telemetry chỉ counter/timing aggregate local, không ID/ảnh. |
