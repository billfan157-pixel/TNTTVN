...
- QR không có `sig` trên phiếu điểm/chứng nhận chỉ là mã tham chiếu nội bộ. UI và bản in phải ghi rõ mã đó không phải chữ ký số và không xác nhận điểm số, tính toàn vẹn tài liệu hoặc việc cấp chứng nhận.
...
> **Persist session 2 tầng (ADR-045, amended 2026-09-17)**: localStorage `parish_current_user` chỉ chứa marker không-PII `{id, role, parishId}`; user đầy đủ lưu mã hóa AES-GCM trong IndexedDB (`parish_auth_user`, dexieStorage, tenant-scoped). Marker chỉ là bootstrap/UX guard, không phải transport hoặc sync authority. Khi snapshot local hỏng/thiếu (Dexie purge, key rotate, LAN không có `crypto.subtle`): client gọi `POST /api/auth/refresh` (cookie HttpOnly) rồi **`GET /api/auth/me`** (trả `{id, username, fullName, phone, role, status, parishId}`; không trả `mustChangePassword`) để rebuild. Client chỉ activate/rebuild khi identity server khớp document-local tenant scope/marker; offline → logout sạch.
...
- `POST /api/auth/refresh` — **A-NEW-01/A-NEW-02 (2026-08-10): token CHỈ từ HttpOnly cookie `parish_refresh` — body bị IGNORE hoàn toàn** (JS không bao giờ có refresh token). Rate limit 30/60s/IP:
  - Token hợp lệ → **rotate**: thu hồi token cũ (revokedAt + replacedBy), phát hành access token mới + **set cookie mới**. JSON trả `{ accessToken, userId, parishId }` để client bind kết quả refresh vào identity của document hiện tại; không trả refresh token. Token cũ không dùng lại được.
  ...
  - Request thua atomic claim khi hai refresh dùng cùng credential cũng thực hiện containment này, kể cả thu hồi successor của request thắng. Rotation kiểm lại live account trong transaction; epoch đã thay đổi → `SESSION_INVALID`, không cấp successor từ quyền cũ.
...
- Client `src/lib/api/core.ts` tự động gọi refresh (mutex trong tab) khi gặp 401 — refresh qua cookie (`credentials: 'include'`), **không gửi body token**. Client chỉ cài access token mới khi `{userId, parishId}` trong response khớp document-local tenant scope đã capture; account switch trong tab khác không thể biến refresh của A thành authority của B. Khi Web Locks khả dụng, login/change-password/refresh/logout dùng cùng khóa `catevia-refresh-session` giữa các tab cùng origin để tuần tự hóa thay đổi cookie. Thiếu Web Locks vẫn áp dụng server replay containment. Refresh token **không tồn tại trong JS** (không memory, không localStorage).
...
- Không cần access bearer hợp lệ: possession của cookie `parish_refresh` chỉ cho phép thu hồi đúng refresh session. Body `{ refreshToken }` legacy chỉ dùng nếu không có cookie. Transaction thu hồi cả chuỗi successor của phiên đó nếu refresh vừa commit; không đụng phiên thiết bị khác hay `tokenVersion`.
- Response trong envelope `data` là `{ success: true, serverConfirmed: boolean, sessionRevoked: boolean, message }`. Có refresh credential → `serverConfirmed=true` sau khi transaction hoàn tất; `sessionRevoked=false` nếu credential đó không có row trên server. Không cookie/body token → không xác định được phiên: `serverConfirmed=false`, `sessionRevoked=false`, vẫn xóa cookie và không tự logout toàn bộ. Logout không thu hồi riêng access JWT đã phát hành: token đó vẫn chịu TTL và live account validation.
- **A01 Phase 1 + A-NEW-02**: luôn xóa cookie `parish_refresh` (Max-Age=0); chịu CSRF origin guard như `/refresh`.
- `authStore.logout()` trả `Promise<{ serverConfirmed, snapshotCleared }>`: xóa memory token/marker/UI auth ngay, chờ xóa snapshot mã hóa đúng tenant trước khi bỏ scope và reset stores. Login/bootstrap trong cùng tab chờ logout đang chạy. Lỗi kết nối hoặc response thiếu xác nhận → `serverConfirmed=false` và cảnh báo trên trang đăng nhập; lỗi xóa snapshot → `snapshotCleared=false`, không phục hồi authentication. Không coi logout local là bằng chứng server đã thu hồi phiên.

### JWT secrets (production)
- `JWT_SECRET` và `JWT_REFRESH_SECRET` bắt buộc có giá trị riêng biệt, mỗi secret ít nhất 32 ký tự. Auth module throw trước khi phục vụ request nếu thiếu, ngắn hoặc trùng nhau; lỗi không in giá trị secret. Production không dùng fallback; dev/test vẫn có fallback chủ ý. Việc cung cấp secret ngẫu nhiên đủ entropy thuộc cấu hình triển khai.
...
| `GET /api/reports/classes?academicYear=YYYY-YYYY` | Danh sách lớp được phép dùng cho báo cáo chính thức; year terminal lấy class label/branch từ finalization evidence | admin + chunhiem + phuta (service lọc assignment) | `ReportClassDTO[]` | 409 khi evidence lịch sử thiếu; 500 lỗi không phân loại |
| `GET /api/reports/class-summary/:classId?academicYear=YYYY-YYYY` | Roster/tổng hợp lớp authoritative cho report/print/export | admin + chunhiem + phuta (service kiểm class scope) | `ClassSummaryDTO`; mỗi `students[]` có `gpa: number|null`, grades, `attendanceSummary` đầy đủ và `promotion` đầy đủ để tổng hợp `ReportCardDTO` không mất dữ liệu | 403 ngoài scope, 404 không có cohort/lớp, 409 thiếu evidence lịch sử |
...
- `ReportCardDTO` trả `grades[].classification`, `grades[].scoreDaoDuc` (khi evidence có), `yearSummary { gpa, classification }` và attendance/promotion. Các surface chính thức không tính lại các field học vụ này từ client store. Open year dùng server facts hiện tại; year terminal dùng frozen evidence và fail closed nếu thiếu.
- `ClassSummaryDTO.students[]` giữ GPA chưa có điểm là `null` (không ép thành `0`), giữ nguyên attendance counts/rate và promotion provenance/override. Client chỉ tổng hợp hàng loạt khi đủ các field này; payload legacy/thiếu field phải gọi từng report card thay vì bịa giá trị mặc định.
- Mọi nút xuất bảng điểm trên desktop/mobile/print modal gọi Reporting API rồi tạo `.xlsx`; không được serialize cache/store/editor state. CSV dùng serializer chống spreadsheet-formula injection.
...
| `POST /api/exams/:id/results` | Upsert kết quả (`results[]` — `studentId`, `score`, `essayScore?`, `source`, `examVersion?` A–H, `answers?`, `scanMetadata?` ≤10KB không ảnh; `clientMutationId?`, `attemptFingerprint?`, `capturedAt?`, `expectedResultVersion?`, `afterMutationId?`). Update row hiện hữu bắt buộc gửi đúng `expectedResultVersion`; create chỉ nhận absent/0. `afterMutationId` nối một lệnh đã commit của cùng owner/session/student để lấy ID+version authoritative trước khi update. MC/mixed do server chấm authoritative; command essay-only của mixed giữ nguyên answers, mã đề và provenance phần TN hiện hữu. Source `omr|qr_scan` bắt buộc metadata `detectionStatus='accepted'`; MC/mixed còn phải khớp `examVersion` và `questionCount`. Mutation receipt scope parish+user commit cùng result; retry chính xác sau khi phiên completed vẫn trả duplicate, payload đổi phải conflict. | admin / chunhiem / phuta | `{ session, saved, upserted, total, adjustments, items: [{ clientMutationId?, studentId, status: 'created'\|'updated'\|'duplicate', serverScore, resultVersion, resultId }] }` | 403 ngoài lớp, 400 payload/provenance không hợp lệ, 409 completed, `EXAM_RESULT_VERSION_CONFLICT` hoặc `IDEMPOTENCY_CONFLICT` |
| `DELETE /api/exams/:id/results/:studentId?clientMutationId=&expectedResultId=&expectedResultVersion=` | Xóa đúng result ID+version người dùng đã thấy (chỉ phiên `draft`), audit + receipt trong một transaction. Sau lệnh save offline chưa ACK, thay hai trường expected bằng `afterMutationId` của save trước đó. Retry cùng command trả `duplicate`, không xóa result mới tạo lại. | admin / chunhiem / phuta | `{ deleted: true, studentId, duplicate? }` | 400 thiếu intent, 403 ngoài lớp, 404 không tồn tại, 409 completed/OCC hoặc `IDEMPOTENCY_CONFLICT` |
...
- **Continuous result mutation (ADR-067/107)**: `api.saveExamResults` gửi cùng `Idempotency-Key`, `clientMutationId` và `expectedResultVersion` ổn định trong mọi retry của một request. Offline queue lưu từng item dưới entity `exam_result`/key `sessionId::result::studentId`; hai phần TN/TL của một lệnh chưa gửi được hợp thành payload đầy đủ; lệnh đã claim/retry giữ nguyên bytes và lệnh kế tiếp dùng `afterMutationId` chờ receipt trước. Processing lease hết hạn được phục hồi về retrying. Chỉ mutation thật sự dùng chung queue ID mới supersede ledger cũ; `complete` là barrier kiểm tra hàng đợi bền vững và ledger trước khi finalize cả khi mạng đã trở lại. Stale OCC trả conflict và phải refresh/confirm, không tự replay ghi đè.
- **Offline result read model:** cache kết quả được tách theo `sessionId`; đổi phiên hoặc chọn lại cùng phiên khi offline phải khôi phục đúng cache của phiên đó. ACK cập nhật cả row đang hiển thị và cache của phiên sở hữu; xóa/remap phiên phải xóa/chuyển cache tương ứng.
...
`POST|PUT /api/students` resolve membership branch từ class active. Khi `PUT` thực sự đổi `classId` hoặc `branch`, body bắt buộc `membershipChangeReason` dài 5–500 ký tự; audit action là `UPDATE_MEMBERSHIP_CORRECTION`. Đây là correction hành chính, không phải promotion. Đổi `classId` sang niên khóa khác → `409 CROSS_ACADEMIC_YEAR_TRANSFER_DISALLOWED` (đi Xét Lên Lớp thay thế); tạo/chuyển vào lớp thuộc niên khóa đã khóa/chốt → `409 ACADEMIC_YEAR_INVALID` (tạo lớp mới trong niên khóa khóa/chốt cũng vậy). `parentPhone` validate `/^(\+84|0)\d{9,10}$/` ở cả backend và `StudentModal` (10-11 số đầu 0 hoặc +84). Client StudentModal chỉ đóng/báo đã lưu sau khi durable queue insertion thành công; nếu student mutation bị server từ chối vĩnh viễn, sync giữ failed payload trong Diagnostics và reconcile object từ `GET /api/students/:id` (hoặc bỏ projection không đáng tin nếu authoritative read thất bại).
...
Tất cả route yêu cầu JWT staff baseline (`admin|chunhiem|phuta`); backend tiếp tục resolve capability theo account + active explicit organization `position_code`/unit scope + operation resource role. `position_title` không cấp quyền. Position codes gồm `PARISH_LEADER|PARISH_SECRETARY|PARISH_DEPUTY` (BOARD/toàn xứ), `BRANCH_LEADER|BRANCH_DEPUTY` (BRANCH), `COMMITTEE_LEADER|COMMITTEE_DEPUTY` (COMMITTEE); backend giữ codes tiếng Anh, UI hiển thị nhãn tiếng Việt. Thư ký hẹp hơn văn phòng xứ: đọc toàn xứ + tạo Event Xứ đoàn + toàn quyền nội dung event mình tạo (creator role); không tạo Event chuyên môn, không manage/assign event người khác. U-20 (2026-09-21, product-approved) chốt tầng Mảng của event Xứ đoàn: `task.create`/`task.manage` chỉ mở cho Trưởng/Phó Ban-Ngành đang hiệu lực của đúng đơn vị sở hữu Mảng hoặc `WORKSTREAM_LEAD` của chính Mảng đó — `EVENT_CREATOR`/`EVENT_ORGANIZER` (Trưởng Xứ đoàn, Phó Xứ đoàn, Thư ký, kể cả event do họ tạo) không tạo/sửa task trong Mảng; Trưởng Xứ đoàn được bổ nhiệm Field Lead cho Mảng có `sourceUnitId` (target vẫn bị ép là Trưởng Ban/Ngành đương nhiệm); Phó Ban/Ngành có thêm `task.assign`/`task.reassign` trong unit/descendants nhưng vẫn không `assign_lead`; task không thuộc Mảng (template instantiate/follow-up, dữ liệu grandfather) giữ nguyên hành vi. `GET /permissions?eventId=` của event Xứ đoàn trả `task.create=false` cho người không có quyền tầng Mảng ở bất kỳ Mảng nào, và `GET /permissions?workstreamId=` trả đúng theo từng Mảng (map chỉ là UX; command path tự kiểm lại capability trên resource thật). Technical admin có parish-wide mutation override theo quyết định O2 đã duyệt (trừ execute vẫn cần assignment ACCEPTED của chính actor). Frontend permission response/route policy chỉ dùng để hiển thị UX.

- `GET /events`, `GET /events/public-summary`, `GET /workstreams`, `GET /tasks` và `GET /reminders/inbox` dùng `page`/`limit`, mặc định 50 và từ chối `limit > 500`; response là `{ success, data, meta:{page,limit,total,totalPages}, error:null }`. Với private list, authorization được áp dụng trước khi tính `total`/slice. `GET /events` (W2.13) nhận thêm filter pushdown SQL `q` (substring title/location, tối đa 200 ký tự), `status`, `scope=XU_DOAN|UNIT` — total phản ánh đúng tập khả kiến sau filter; client Operations debounce 300ms và không ghi danh sách đã lọc vào offline cache. `GET /events` (U-19, 2026-09-21) trả kèm `createdByName`/`organizerName` (name-only, resolve server-side trong cùng parish, `null` khi tài khoản/person không còn resolve được) để pane "Sự Kiện & Công Việc Đang Diễn Ra" hiển thị người tạo & người phụ trách mà không phải gọi detail từng dòng; `GET /events` (U-19b) sắp xếp theo lịch: sự kiện chưa kết thúc (`endsAt >= now`) đứng trước theo `startsAt` tăng dần — gần nhất lên đầu — sự kiện đã kết thúc theo sau, mới nhất trước, nên phân trang giữ sự kiện sắp diễn ra ở trang 1; client áp lại đúng quy tắc này cho hàng cache/offline và sắp khung theo loại nào có sự kiện gần nhất. `GET /events/:id` trả thêm `creator:{userId,displayName}` bên cạnh `organizer`. Enrichment chỉ chứa họ tên — không username, phone, email hay trạng thái account. `GET /events/:id`, `/readiness`, `/headcount` là tenant/resource-scoped detail; detail gồm workstreams, tasks, participants, `retrospective|null`, permission map và readiness computed-on-read.
- `GET /candidates?taskId=...|workstreamId=...|eventId=...` yêu cầu đúng một resource và capability quản lý tương ứng trước khi trả dữ liệu. Projection chỉ gồm `{parishId,personId,userId,displayName,eligibility,inResourceScope}`; không trả phone, username, tiểu sử, chức danh hay trạng thái account thô. Với Trưởng ngành/Trưởng ban và resource lead có unit scope, server chỉ trả person có nhiệm kỳ hiệu lực trong unit/descendants; chỉ admin override mới nhận directory toàn xứ (bypass target-scope). Trưởng/Phó/Thư ký Xứ đoàn không còn assign qua position nên `GET /candidates` với task/workstream đòi assign trả 403 cho họ; directory scoped theo unit khi họ kiêm nhiệm Trưởng unit. `eligibility=PLANNING_ONLY` là person chưa có account, không đồng nghĩa có thể nhận/duyệt/được nhắc.
- `POST /events`, `PUT /events/:id`, `POST /events/:id/transition`: event aggregate và writer duy nhất. Body tạo nhận `eventScopeType?: XU_DOAN|UNIT` (thiếu thì suy từ `scopeUnitId`: NULL→XU_DOAN, có→UNIT; sai khớp trả `OPERATION_EVENT_SCOPE_TYPE_MISMATCH`), `organizerUserId xor organizerPersonId`. Creator và organizer là hai khái niệm khác nhau: sự kiện Xứ đoàn có organizer bắt buộc là Xứ đoàn trưởng active (thiếu thì server tự gán khi đúng một người, sai thì `ORGANIZER_MUST_BE_PARISH_LEADER`); Trưởng Xứ đoàn chỉ được tạo event Xứ đoàn (tạo event chuyên môn bị `403 FORBIDDEN` ở capability, kể cả khi chỉ định đúng Trưởng unit) và chỉ đứng tên event Xứ đoàn — đổi organizer event chuyên môn sang chính mình bị `400 ORGANIZER_MUST_BE_UNIT_LEADER`. Phó Ban/Phó Ngành tạo sự kiện chuyên môn phải chỉ định Trưởng cùng unit (`ORGANIZER_REQUIRED`). Sự kiện chuyên môn phải thuộc đúng unit của người tạo (`UNIT_SCOPE_MISMATCH`), trừ admin (Trưởng/Phó Xứ đoàn/Thư ký chỉ tạo Event Xứ đoàn — tạo chuyên môn luôn 403; menu "+ Tạo mới" ẩn hẳn option unit với ba vai này, Trưởng Xứ đoàn giữ picker unit cho task độc lập). Client không gửi/sửa `sourceParishEventId`; nếu gửi server trả `CALENDAR_LINK_SERVER_MANAGED`. Mọi create/update chạm visibility `PUBLIC_SUMMARY` cần capability riêng `operations.event.publish_public`; hiện chỉ Trưởng Xứ đoàn có capability này ngoài admin override mọi môi trường theo O2 (ADR-112). DRAFT không tạo projection. Lần đầu DRAFT→PLANNING mới upsert `parish_events` và ghi durable targeted parent notification; `INTERNAL` không có projection. Chuyển public→internal, CANCELLED hoặc lùi DRAFT soft-delete projection. Hủy event đang public cũng cần publish capability. Transition dùng `version`; PLANNING→PREPARING có structured warning `TASK_ACCEPTANCE_PENDING`, READY/LIVE thủ công có blocker/override reason. Manual COMPLETED cần `outcomeSummary` và toàn bộ task required DONE, không cần retrospective. Event fields không sửa từ LIVE (`409 EVENT_IMMUTABLE`); completed/cancelled event không nhận planning child mới. Đổi scope hoặc organizer cần organizational create authority ở scope đích; đổi `scopeUnitId` tự dẫn `eventScopeType` theo (`OPERATION_EVENT_SCOPE_TYPE_MISMATCH` khi cố tình lệch), đổi organizer kiểm lại rule leader cho business actors (W2.1: UI create/edit nay gửi được `description`, `expectedHeadcount` và đổi `organizerUserId` — body PUT gửi kèm `organizerPersonId: null` khi swap sang user organizer). Completion dùng `completionRecordId` thuộc aggregate để reopen/recomplete cập nhật đúng `ACTIVITY`, không suy ownership từ soft/nonunique `source_event_id`.
...
- `GET /creation-options`: menu "+ Tạo mới" server-authoritative, trả `{canCreateXuDoanEvent, xuDoanOrganizers:[{userId,displayName,positionCode}], units:[{id,name,unitType,canCreateEvent,canCreateTask,organizers,myRole}]}` chỉ gồm hành động caller được phép; UI chỉ hiển thị đúng các option này, mutation routes kiểm tra lại. `POST /tasks` nhận thêm `scopeUnitId?`; task độc lập (không event/workstream) bắt buộc có scope (`STANDALONE_TASK_SCOPE_REQUIRED`, trừ tương thích admin cho dữ liệu cũ) và scope được backfill từ workstream/event khi thiếu. Task gắn graph có scope đơn vị cụ thể phải ghi đúng scope của graph (`scopeUnitId == workstream.source ?? event.scope`, hardening 2026-09-16 theo BUSINESS_RULES #9/O6 — lệch scope mà không có authority ở scope đích trả 403, kể cả khi caller có quyền ở scope khai báo). Task trong event Xứ đoàn bắt buộc `workstreamId` (`TASK_WORKSTREAM_REQUIRED`, trừ admin và task cũ grandfather). Field trong sự kiện Xứ đoàn bắt buộc `sourceUnitId` (`FIELD_SCOPE_REQUIRED`, trừ admin); Field Lead phải là Trưởng Ban/Trưởng Ngành đương nhiệm đúng đơn vị — Phó, member thường, Trưởng đơn vị khác và Xứ đoàn trưởng đều bị chặn (Trưởng Xứ đoàn không delegate nên `FORBIDDEN`, các trường hợp còn lại `WORKSTREAM_LEAD_OUTSIDE_UNIT`). Menu tạo mới: Trưởng Xứ đoàn chỉ tạo event Xứ đoàn + task độc lập (`units[].canCreateTask` giữ cho picker standalone, `canCreateEvent` chỉ admin/unit role); Phó/Thư ký chỉ event Xứ đoàn.
- `/tasks/:id/assign`, `/tasks/:id/handover` và `/workstreams/:id/members` revalidate target scope trong cùng command transaction. Trưởng ngành/Trưởng ban chỉ chọn được người có service term hiện hành trong unit phụ trách hoặc descendants; gọi API trực tiếp ngoài phạm vi trả `403 TARGET_OUTSIDE_ORGANIZATION_SCOPE` và không tăng aggregate version. Chỉ admin override mới bypass target-scope; Trưởng/Phó/Thư ký Xứ đoàn không delegate qua position scope (thiếu capability trả `403 FORBIDDEN` trước khi tới kiểm tra scope). Creator/Organizer/Lead manage nội dung nhưng không tự động assign (muốn giao phải đồng thời là Trưởng unit hiện hành). Picker là UX, không phải authorization boundary. Trưởng Xứ đoàn View toàn xứ (kể cả DRAFT Xứ đoàn) và sửa nội dung Xứ đoàn kể cả do Phó/Thư ký/admin tạo, nhưng không giao người.
