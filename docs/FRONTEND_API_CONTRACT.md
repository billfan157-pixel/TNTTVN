...
| `POST /api/exams/:id/results` | Upsert kết quả (`results[]` — `studentId`, `score`, `essayScore?` (chỉ phiên mixed — điểm phần tự luận), `source`, `examVersion?` A–H default A, `answers?`, `scanMetadata?` ≤10KB không ảnh). Với MC scan, server chọn key đúng version và tự tính lại điểm; version thiếu key hoặc metadata khác `accepted` bị chặn. Với mixed: server tổng hợp `score = clamp(Σ points câu TN đúng + essayScore đã lưu, 0, maxScore)`, merge 2 pha giữ nguyên `answers`/`essay_score`; thiếu cả hai thành phần → 400; phiên không phải mixed gửi `essayScore` → 400. | admin / chunhiem / phuta | `{ session, saved, upserted, total, adjustments }` | 403 ngoài lớp, 400 điểm/answers/version/metadata/thiếu nhi không hợp lệ, 409 completed |
