...
| `POST /api/exams/barcode/decode` | Decode barcode text (`tntt-exam:{sessionId}:{studentId}`) — fallback khi QR fail (ADR-043) | auth | `{ sessionId, studentId, classId, subject, examType, maxScore, questionCount }` | 400 barcode không hợp lệ, 403 ngoài lớp, 404 phiên không tồn tại |
...
| **Code Scan Pipeline** | `qr.ts` + `cameraFrame.ts` + `examCodeScanner.ts` + `ExamScanModal.tsx` | QR render bằng ma trận thư viện trong hệ module chuẩn, quiet zone 4 module. Camera ưu tiên 1920×1080 và canvas chỉ lấy source crop tương ứng `object-fit: cover` của viewport portrait; sau đó thử QR crop/toàn frame → Code128 fallback → OMR. UI luôn báo searching, code not found, invalid TNTT code, code found but OMR failed, hoặc detected. Code128 dùng viewBox động, dải full-width cuối phiếu và decoder multi-row. |
