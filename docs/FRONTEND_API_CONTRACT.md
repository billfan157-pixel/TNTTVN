...
| **Code Scan Pipeline** | `qr.ts` + `cameraFrame.ts` + `examCodeScanner.ts` + `ExamScanModal.tsx` | QR render bằng ma trận thư viện trong hệ module chuẩn, quiet zone 4 module. ID production dùng payload Alphanumeric lossless `TE:XXXXXXXX:XXXXXXXX` để giảm 29→21 module/cạnh; payload legacy vẫn parse được. Camera ưu tiên 1920×1080 và canvas chỉ lấy source crop tương ứng `object-fit: cover` của viewport portrait; scanner thử crop trên/phải trước, thêm unsharp luma nhẹ cho ảnh mất nét, rồi toàn frame → Code128 fallback → OMR. UI luôn báo searching, code not found, invalid TNTT code, code found but OMR failed, hoặc detected. Code128 dùng viewBox động, dải full-width cuối phiếu và decoder multi-row. |
