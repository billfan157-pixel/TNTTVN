...
- `GET /permissions?unitId=&eventId=&workstreamId=&taskId=`: capability map server-authoritative `{parishId,permissions}` cho đúng một resource graph; nhiều ID không cùng graph trả toàn bộ `false`. Chỉ dùng để hiển thị UX, mutation routes kiểm tra lại. `GET /units`: active org units mà caller hiện có `operations.workstream.create` (authorize trước pagination); client dùng cho scope picker, không thay thế server check.
