...
- `POST /api/auth/refresh` — **A-NEW-01/A-NEW-02 (2026-08-10): token CHỈ từ HttpOnly cookie `parish_refresh` — body bị IGNORE hoàn toàn** (JS không bao giờ có refresh token). Rate limit 30/60s/IP:
  ...
  - **CSRF guard (A-NEW-02)**: vì production cookie gửi trong cross-site POST, `/refresh` + `/logout` kiểm tra `Origin` header — có `Origin` mà không nằm trong CORS allowlist (tnttvn.vercel.app + localhost dev) → `403`. Origin rỗng (curl/server-to-server) cho qua.
...
| `GET /api/reports/report-card/:studentId?academicYear=YYYY-YYYY` | Phiếu điểm của con (grades 2 HK, attendanceSummary, promotion) — phụ huynh chỉ xem được con mình qua `CanAccessStudentSpecification` | auth (mọi role, quyền truy cập qua spec) | `ReportCardDTO` | 403 không sở hữu, 404 không tìm thấy |
...
| Method & Path | Purpose | Auth | Success `data` | Errors |
| :--- | :--- | :--- | :--- | :--- |
| `POST /api/exams` | Tạo phiên chấm (`classId`, `subject`, `scoreType` oral/15m/1period/midterm/final, `maxScore` 1-10 default 10, `semester`, `academicYear?`, `examType?` `written/multiple_choice`, `questionCount?` (1–50 câu, **bắt buộc khi `examType='multiple_choice'`**), `answerKey?` (JSON `Record<number,'A'\|'B'\|'C'\|'D'>` tối đa 50 câu — **server validate format**: JSON hợp lệ, key là số nguyên 1..questionCount, value A-D, non-empty; `idempotencyKey?` — Phase 3/4) | admin, chunhiem, phuta (lớp mình) + `checkUserClassAccess` | `ExamSession` (`EXS-`, `status:'draft'`); **retry cùng `idempotencyKey` trả session ĐÃ TẠO (không duplicate)** | 403 ngoài lớp, 404 lớp không tồn tại, 400 answerKey không hợp lệ / questionCount ngoài 1-50 / thiếu questionCount khi trắc nghiệm |
...
| `POST /api/exams/:id/results` | Upsert kết quả (`results[]` — `studentId`, `score` 0-`maxScore`, `source` `qr_scan/omr/quick_entry`, `answers?` JSON string map câu → đáp án MC) — **phuta check class access ngay tại đây** | admin / chunhiem / phuta | `{ session, saved, upserted, total }` | 403 ngoài lớp, 400 điểm/thiếu nhi không hợp lệ, 409 phiên đã hoàn tất |
...
| `GET /api/exams/:id/results` | Kết quả phiên kèm `studentCode/studentName/holyName` (+ `answers` per student — ADR-024) | auth theo class | `{ session, results }` | 403 ngoài lớp |
| `POST /api/exams/:id/complete` | Đóng phiên — validate semester lock + audit `EXAM_FINALIZE`; **idempotent**; server hiện chỉ lưu; client tự thực hiện finalize grades | admin, chunhiem | `ExamSession` (`status:'completed'`) | 403 ngoài lớp / 403 semester locked / 409 chưa có kết quả |
...
| `POST /api/exams/barcode/decode` | Decode code text: production compact `TE:{sessionHex8}:{studentHex8}` hoặc legacy `tntt-exam:{sessionId}:{studentId}` — fallback khi QR fail (ADR-043) | auth | `{ sessionId, studentId, classId, subject, examType, maxScore, questionCount }` | 400 barcode không hợp lệ, 403 ngoài lớp, 404 phiên không tồn tại |

- **Server không tự ghi `grades`**: mọi điểm (miệng/15'/1 tiết qua pipeline daily; Giữa Kỳ/Cuối Kỳ qua `_source='exam_scan'`) được client commit — đi qua `gradeService.upsertGrade` (OCC + semester lock + class access + audit) như các luồng khác.
...
| **Code Scan Pipeline** | `qr.ts` + `cameraFrame.ts` + `examCodeScanner.ts` + `examScanIdentity.ts` + `ExamScanModal.tsx` | QR render bằng ma trận thư viện trong hệ module chuẩn, quiet zone 4 module; QR đề gộp 120px. ID production dùng payload Alphanumeric lossless `TE:XXXXXXXX:XXXXXXXX` để giảm 29→21 module/cạnh; payload legacy vẫn parse được. Camera ưu tiên 1920×1080 và canvas chỉ lấy source crop tương ứng `object-fit: cover` của viewport portrait; scanner thử crop tập trung trên/phải (raw, phóng 2×, unsharp) trước toàn frame → Code128 fallback. Định danh đúng phiên được khóa 5 giây giữa các frame để QR và OMR không phải thành công đồng thời trong một ảnh; hết TTL scanner bắt buộc đọc mã lại. Trong thời gian khóa, bỏ bước decode mã để dành CPU cho OMR. UI hiển thị riêng `Mã phiếu` và `Khung OMR`; chỉ chuyển sang detected khi OMR hợp lệ. Code128 dùng viewBox động, dải full-width cuối phiếu và decoder multi-row. |
| **Stable Mobile Grading** | `GuidedGradeModal.tsx` + `ExamSessionView.tsx` + `ExamScanModal.tsx` | Nút **Chấm Ổn Định** mở wizard mobile: tìm/chọn học sinh trong `classStudents` của phiên, hiển thị điểm đã lưu, rồi chọn `Chỉ quét OMR` (fixed identity, không decode QR, source=`omr`) hoặc nhập điểm trực tiếp (source=`quick_entry`). Điểm client validate 0..maxScore và server tiếp tục validate/authorize theo session class; cả hai đường dùng `examStore.saveScores`, nên giữ offline queue/upsert hiện có. |
...
| **OMR Fail Reasons** | `ExamScanModal.tsx` | Vietnamese tooltips for 10+ OMR failure codes; trạng thái xanh xác nhận mã đã được giữ 5 giây và nêu riêng lý do OMR chưa đạt, trạng thái vàng dành cho mã chưa thấy/không hợp lệ |
...
| Endpoint | Mô tả | Quyền | Response | Lỗi |
| :--- | :--- | :--- | :--- | :--- |
| `POST /api/grades/undo-import` | Body `{ semester: 1\|2, academicYear: string, studentIds: string[] (1–500) }` → đảo ngược lần ghi điểm gần nhất của từng bảng điểm trong đợt import (audit entry mới nhất `entityType='grade'`: `CREATE` → xóa row, `UPDATE` → khôi phục `oldValue`). Trả `{ results: [{ studentId, status, message? }] }` | admin (toàn parish), chunhiem (chỉ lớp được bổ nhiệm — check trong tx, ADR-016 S24) | 200 `{ success, data: { results } }` | 400 zod (semester/studentIds), 401/403, 409 không dùng |
