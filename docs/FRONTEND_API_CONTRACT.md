...
| **Code Scan Pipeline** | `qr.ts` + `cameraFrame.ts` + `examCodeScanner.ts` + `examScanIdentity.ts` + `omrScanConsensus.ts` + `ExamScanModal.tsx` | QR render bằng ma trận thư viện trong hệ module chuẩn, quiet zone 4 module; QR đề gộp 120px. ID production dùng payload Alphanumeric lossless `TE:XXXXXXXX:XXXXXXXX` để giảm 29→21 module/cạnh; payload legacy vẫn parse được. Camera ưu tiên 1920×1080, continuous-focus khi thiết bị hỗ trợ, và canvas chỉ lấy source crop tương ứng `object-fit: cover` của viewport portrait; scanner thử crop tập trung trên/phải (raw, phóng 2×, unsharp) trước toàn frame → Code128 fallback. Luồng UI tách thành hai bước: đưa QR lại gần, rồi lùi máy để căn OMR; định danh đúng phiên được khóa 20 giây giữa các frame. Trong thời gian khóa, bỏ decode mã để dành CPU cho OMR. Auto scan chỉ chuyển sang detected khi hai frame OMR liên tiếp có cùng fingerprint đáp án trong tối đa 1,8 giây. Code128 dùng viewBox động, dải full-width cuối phiếu và decoder multi-row. |
