...
## Audit TELEGRAM-20260908 — channel retirement (implemented; chưa deployed)

- Bot/chat authority, global admin-chat fallback, parent linking/consent UI and Telegram runtime delivery are removed under ADR-111. The old parent routes remain authenticated parent-only tombstones returning `410 CHANNEL_RETIRED`, with no read/write or new audit action.
- Migration `20260908-235` plus startup recovery consume open tokens, revoke active/enabled links and terminalize retrying Telegram notification rows. Sent/failed rows, revoked link history and legacy audit labels are retained; no destructive cleanup or replay occurs.
- Smart, parish, Sunday and leave-review messages use targeted Web/Native Push. Child-sensitive delivery continues to intersect original recipients with current same-parish ACTIVE/nondeleted ownership at provider time. Grade override/restore remains transactionally audited without an external notification.
- Local source/test evidence is recorded in [Telegram decommission research](TELEGRAM_DECOMMISSION_RESEARCH_2026-09-08.md). Live secret removal, provider bot-token revocation, production migration counts and physical browser/Android/iOS push smoke remain external gates.

## Audit XD-20260907 — cross-domain remediation (partial; chưa deployed)

- Follow-up XD-06: scope-aware Grade/Attendance pulls resolve current authority + complete scope + data in one transaction, force full data on scope changes, and durably retract replaceable caches without deleting queued intent. Stale-session/older response, quota failure, legacy-cache migration and grant/revoke regressions are tracked in report §0.3. Offline devices, exports and other domain caches are not certified erased.
- Follow-up XD-09: standalone SQLite backup now fails closed on VACUUM/publish failure; restore preflight requires the full table set and readback verifies content, not only counts/FKs. Fresh-target preparation removes only verified migration-created fund seeds. Local encrypted full-schema drill preserves finalized academic evidence and promotion receipts. Partial JSON restore now rejects protected lifecycle and excluded dependency/provenance before destructive work; successful restore advances the tenant client-data generation and writes success audit in its transaction. Current device blocks on own unsettled queue, then resets/logout after ACK; other and legacy-unmarked devices reset before old queue replay. Production R2/key/restore/cutover and legacy reconciliation remain external gates. Report §0.3–0.4 distinguishes this from full recovery.

- Follow-up XD-02/XD-03: finalized policy/cohort/effective report evidence is persisted atomically, validated on historical reads, and consumed by promotion evaluation/approval. Current auth/parent ownership and soft-delete checks remain authoritative; no historical privilege snapshot. Legacy missing/invalid evidence conflicts rather than silently using today's settings/membership. Migrations 179–181 are nullable with no inferred backfill; persisted fee facts stay in the recorded class/year/type after membership changes. See report §0.2 for regression evidence and unresolved production/legacy recovery gates.
- R0 follow-up adds a read-only, privacy-minimized reconciliation command for protected-year evidence and daily Grade↔ledger mismatch; it excludes active manual overrides and performs no repair. Import undo also treats a parent account created/relinked after import as downstream activity when undo would change that phone ownership link. Current evidence and limits are recorded in report §0.5; production inventory/backfill remain unexecuted.
- Full coverage exposed one transient commit-time `SQLITE_BUSY` in Grade override while 329/330 files passed. Isolated normal/coverage reruns passed; trace found override/restore bypassed the standard bounded transaction wrapper. Both commands now use `runDbTransaction`; notification remains post-commit so a retry cannot enqueue twice. The original failed run remains recorded; the final serialized rerun passed **331/331 files, 2,360/2,360 tests** with coverage **71.21/60.60/64.17/73.84** (statements/branches/functions/lines).

- Baseline và current disposition tách riêng ở [cross-domain report §0](cross-domain-integration-invariant-audit-2026-09-07.md#0-remediation--working-tree-chưa-deployed). Không chứng nhận toàn bộ chín finding đã đóng.
- XD-05: leave review revalidates captured recipients against current ACTIVE parent/child ownership, cùng resolver của notification queue; chỉ gửi generic academic-update body. Delivery vẫn best effort, không phải durable outbox. Current/relinked/locked/deleted recipient controls đã PASS.
- XD-04: Attendance/leave approval kiểm date-based persisted ranges, protected lifecycle state và canonical/legacy semester-lock IDs trong write transaction; year-create API không nhận range vượt canonical Aug–July. **7/7** period-lock regressions PASS; không tự sửa historical ranges.
- XD-08: manual ledger mutation + server-derived full-ledger Grade projection atomic, manual override được giữ, client không là derived-score authority; Grade-only undo không revert daily projection. Rollback, duplicate, legacy-client, override và undo regressions PASS. Historical reconciliation chưa thực hiện.
- XD-09 containment: partial JSON restore từ chối protected lifecycle/snapshot và excluded dependency/provenance trước safety-write, kiểm lại trong transaction trước delete; generation + success audit atomic với restore. Guard gồm staff/service assignment, attendance session, notification, manual financial ledger, grade-import receipt/mapping memory và legacy exam-question snapshot để không cascade/rebind facts ngoài profile. Backup/restore security suite **10 files / 70 tests PASS**; targeted generation/UI/sync/restore suite hiện tại **5 files / 41 tests PASS**; expanded dependency regression **11/11 PASS**. Import undo cũng chặn manual financial activity sau import (**4 files / 32 tests PASS**). Chưa có full semantic recovery closure, production drill hoặc repair historical loss.
- Final local gate cho follow-up restore/import: security-critical **7 files / 73 tests PASS**; final serialized coverage **331/331 files, 2,360/2,360 tests PASS**; coverage **71.21/60.60/64.17/73.84**. Current lint PASS không warning; DS **0/134**; inventory **31/6/52/12/57**; root TypeScript + server/frontend/PWA production build PASS. Diff check của final snapshot được ghi trong report §0.5. Đây chưa phải production qualification.
- Follow-up XD-01/XD-07: promotion completion receipt commits with membership, shared list/retry/archive predicate rejects approval-only; nullable migrations 177/178 deliberately do not certify legacy rows. Encrypted owner-scoped CREATE ACK stays in queue until local remap succeeds; failure stops child sends, durable ACK replay avoids another CREATE, compaction/pruning preserve journal. Source, synthetic fault regressions and recovery limits at report §0.1; no production migration/inventory or physical crash qualification yet.
- Verification đợt đầu: security gate **7 files / 73 tests PASS**; full suite 323/324 files, 2,278/2,279 tests PASS (metadata test đã sửa/rerun 7/7, full run exit 1 nên coverage gate chưa xác nhận). Follow-up receipt/ACK: security gate có 6 files PASS, auth-cookie 2 timeout ở 5s khi build cùng chạy; rerun auth-cookie nguyên timeout sau build **7/7 PASS**. Promotion wrong-target regressions cũng PASS; client/server build, typecheck, lint và inventory PASS. Không coi đây là full CI/release certification; current remaining scope ở report §0.1.
...
| :--- | :--- | :--- | :--- | :--- |
| TELEGRAM-20260908 | P1/P2 | Chat authority/privacy, restored retry delivery, duplicate notification channel | ✅ Source retirement implemented; production secret/provider/migration/device gates remain | 2026-09-08 (engineering) |
...
| FE-10 | 🟠 P2 | Gộp quản lý lớp vào route staff có thể làm lộ mutation controls cho GLV; roster parish-wide không được làm mở exam controls ngoài assignment | ✅ ENGINEERING VERIFIED (2026-09-01, ADR-090/076 amendment) | parish-wide `GET /students` + redacted/marked `GET /classes`, admin-only roster controls/class mutations, exam-scope + class-write regression |
| AUTHZ-STUDENTS-1 | 🔴 P1 | Roster parish-wide thiếu role middleware làm `phuhuynh` đọc được list/detail PII thiếu nhi cùng giáo xứ | ✅ CLOSED / ENGINEERING VERIFIED (2026-09-01, ADR-092) | staff-only middleware trên cả hai GET; parent list+detail negative test; go-live/smoke/tenant suites |
...
| PERF-XLSX-1 | ⚪ P4 (perf) | xlsx (~400KB) static-import ở 8 module → nằm sẵn chunk Students/Grades/Reports dù user không mở import/export | ✅ CLOSED (2026-08-24; PWA follow-up 2026-09-01): `lib/xlsxLoader.ts` lazy-load + cache promise; **chuyển đủ 8/8 module** (reportExporter, attendanceAnalyticsService, excelImporter, excelTemplateBuilder, ExcelImportModal, ExcelGradeImportModal, examParser, examExporter); production code 0 static xlsx import (còn `import type` trong excelTemplateBuilder — type-only, không vào bundle). `injectManifest.globIgnores` nay loại `xlsx-*.js` khỏi PWA install-time precache; sau lần dùng đầu, static runtime cache vẫn hỗ trợ offline. Callers async hoá giữ nguyên error UX ADR-018 | `src/lib/xlsxLoader.ts` (NEW), `vite.config.ts`, `services/reportExporter.ts`, `services/attendanceAnalyticsService.ts`, `utils/excelImporter.ts`, `utils/excelTemplateBuilder.ts`, `utils/examParser.ts`, `utils/examExporter.ts`, `ExcelImportModal.tsx`, `ExcelGradeImportModal.tsx`, `ExamImportModal.tsx`, `ExamExportModal.tsx`, `ExamPaperModal.tsx`, `xlsxLoader` callers |
...
| SYNC-DELTA-2 (ADR-094) | 🔴 P1 data integrity | Reload full-pull không dùng durable cursor; delta student/class thiếu bounded server snapshot và tombstone; store có thể nuốt lỗi rồi vẫn tiến cursor; localStorage lease không atomic giữa tab | ✅ CLOSED (2026-09-01): server watermark + `updatedAfter..updatedBefore`, stable paging/tombstones, tenant/user Dexie cursor commit-after-all-success, fail-fast store mode, Web Locks primary + expiring lease fallback. Empty cache force full repair. Server OCC/idempotency vẫn là authority | `server/src/routes/sync.ts`, `studentService.ts`, `classService.ts`, `src/lib/{syncCursor,syncLease}.ts`, `src/hooks/useSyncEngine.ts`, stores + tests |
| EXAM-STUDIO-1 (ADR-094) | 🔴 P1 data integrity | A–H key map không chứng minh câu/option đã đảo; đổi nhãn đề B–H trên cùng content có thể chấm sai có hệ thống | ✅ CLOSED for immutable core (2026-09-01): migration `20260901-148`, deterministic server manifest 1–8 mã, materialized content/order/key/hash/seed, one-shot lock, reject positional/duplicate options, legacy question paper fail-closed A, exact manifest print B–H. Combined one-click PDF pack còn backlog không ảnh hưởng score integrity | `server/src/services/examVariantManifest.ts`, `examService.ts`, `routes/exams.ts`, `src/components/exam/{ExamVariantsModal,ExamPaperModal}.tsx`, `src/lib/examVersionPolicy.ts` |
| PERF-EXAM-CHUNK-2 (ADR-094) | ⚪ P4 performance | `ExamSessionView` route chunk 377.04 KB / 116.01 KB gzip dù phần lớn modal chỉ mở theo thao tác; PWA precache mặc định vẫn tải SheetJS dù source đã dynamic import | ✅ CLOSED (2026-09-01): 7 heavy modal/analytics surfaces chuyển React.lazy/Suspense; route chunk còn 54.44 KB / 14.51 KB gzip. `xlsx-*.js` bị loại khỏi install-time precache và chỉ runtime-cache sau lần dùng đầu. Không claim FPS/device performance từ bundle size | `src/components/exam/ExamSessionView.tsx`, `vite.config.ts`, production build artifact |
| OMR-WORKER-1 (ADR-094) | 🟡 P3 research gate | Đề xuất Worker/OffscreenCanvas/SharedArrayBuffer + adaptive threshold chưa có exact target-device bottleneck/accuracy corpus; triển khai sớm có thể hạ fail-closed detector và tăng isolation deployment cost | ⛔ BLOCKED BY EVIDENCE: giữ Scan Engine v4. Chỉ reopen khi ADR-060/068/069 có corpus ≥400 và run 30/100 exact release/device chứng minh main-thread bottleneck cùng non-regression false accept/review/accuracy | ADR-060/068/069, `omrSequenceQualification.ts`, `docs/BUSINESS_RULES.md` §21.3 |
...
| CSP-TOOLBAR-1 | 🟠 P2 (prod console + CSP surface) | Vercel Toolbar (script `feedback.js` từ `vercel.live`, edge inject cho viewer đã đăng nhập Vercel) dựng UI bằng `<style>` element → mỗi lần tải production sinh 6× `Applying inline style violates ... style-src ... The action has been blocked.`; muốn toolbar chạy phải hạ `style-src 'unsafe-inline'` (Vercel xác nhận không hỗ trợ strict CSP) → xung đột A-NEW-23; allowance còn mở XSS surface từ origin thứ ba | ✅ FIXED (repo, 2026-09-23): gỡ allowance `vercel.live`/`assets.vercel.com` khỏi CSP `vercel.json`; contract test pin `script-src`/`style-src` nội bộ + cấm host third-party; post-deploy smoke fail nếu CSP production còn `vercel.live`. ⚠️ Operator: đặt Vercel Toolbar **Off cho Production** (repo không đổi được project settings) | `vercel.json`, `src/__tests__/deploymentSecurityContract.test.ts`, `.github/workflows/deploy-production.yml`, `docs/DEPLOYMENT_GUIDE.md` §7.0, `docs/02_ARCHITECTURE.md` |
...
## Audit ADR-092 — Comprehensive hardening — ✅ ENGINEERING VERIFIED / EXTERNAL GATES EXPLICIT (2026-09-01)

- **Authorization/PII:** `GET /api/students` và `/:id` khóa role staff; parent list+detail đều 403. Staff parish-wide read, tenant binding và class-scoped write không đổi.
- **Web/runtime:** Vercel static HTML có CSP đồng bộ A-NEW-23, clickjacking/nosniff/referrer/permissions headers; theme boot external; health rewrite trả backend JSON; refresh cold-start có timeout 65 giây.
- **Native privacy:** backup + D2D fail-closed, FileProvider app-specific, camera-only permission, Catevia label đồng bộ. Android merged manifest và debug APK đã build thật; iOS source/workflow loại microphone declaration không dùng.
- **Performance/dependencies:** logo auth/lock giảm 1,288,526 → 63,897 byte; production audit 0 vulnerability. Bảy moderate dev-only được giữ residual thay vì override/rollback toolchain thiếu bằng chứng an toàn.
- **Evidence:** security-critical **7 files / 72 tests**; full serialized + coverage **279 files / 1,944 tests PASS**; coverage 69.90/59.93/63.23/72.26; oxlint, DS 0/119, TypeScript, frontend/server build PASS; Android **153 tasks PASS**.
- **Residual/external:** post-deploy Vercel header/browser smoke, physical camera/biometric, signed mobile releases, Render always-on cost/SLO, OMR field corpus và legal retention policy vẫn NOT CONFIRMED. Không tuyên bố production/device acceptance từ local gates.

---

## Audit E2E-SEC-01 — Critical business outcome regression gate — ✅ ENGINEERING VERIFIED (2026-09-01, ADR-093)

- **Finding:** suite browser cũ có isolated backend/database tốt nhưng phần lớn chỉ kiểm render/navigation; parent data ownership, valid-shaped backend deny, persisted reload, semester lock, server scoring và reconnect sync chưa tạo một release gate thống nhất.
- **Controls:** synthetic parent A/B, role sessions thật, API-assisted arrange, UI mutation, authoritative API/reload assertion; access token vẫn memory-only; test artifacts không dùng production data. CI fail nếu test chỉ pass sau retry.
- **Negative authority:** Parent bị 403 ở staff roster/detail; Phụ tá bị 403 finance/class create; Parent bị 403 grade write; Chủ nhiệm ngoài assignment bị 403 student write; dữ liệu mục tiêu được đọc lại để chứng minh mutation không lọt qua.
- **Data integrity:** class/student/grade/finance read-back, semester lock denial, offline attendance queue→batch→database→queue-empty và Smart Exam server recomputation/finalization được bảo vệ. Physical OMR/device behavior vẫn NOT CONFIRMED.
- **Verification:** critical **10/10 PASS** (44,7 giây); full Playwright **72/72 PASS** (5,4 phút), không skip/retry, sandbox cleanup thành công. Full Vitest diagnostic trước fixture fix đạt 1.943 PASS/5 FAIL; hai file bị ảnh hưởng đã chạy lại **21/21 PASS**. Không suy diễn physical-device/field acceptance từ bằng chứng này.
- **Rollback:** R1, không schema/migration/production data.

---

## Audit NATIVE-PUSH-1 — Authenticated, tenant-safe native push — 🟡 ENGINEERING VERIFIED / DEVICE GATE OPEN (2026-09-02, ADR-095)

- **Threats closed:** không dùng Service Worker trong native WebView; token bind theo UUID installation + JWT user/parish; đổi account không giữ người nhận cũ; unregister khác user/parish không xóa được; soft-delete account thu hồi token; queue targeted không degrade broadcast; external notification URL bị loại.
- **Privacy:** raw FCM/APNs token không persist client, không audit/log; audit registration chỉ platform. Token vẫn là plaintext cần thiết trong Turso để provider gửi — database/backup access control là residual boundary. Firebase service account và APNs `.p8` chỉ qua secret manager.
- **Fail-closed:** schema readiness yêu cầu migration/table/index/PK; không provider nào cấu hình thì queue failed `PUSH_PROVIDER_NOT_CONFIGURED`; provider platform thiếu cấu hình được báo skipped. Codemagic Android từ chối build push-capable release khi thiếu base64 `google-services.json`.
- **Evidence cuối:** frozen full regression **290 files / 1.988 tests PASS**; focused provider/fan-out/tenant **5 files / 21 tests PASS**; frontend/PWA + server build PASS; oxlint PASS; DS lint **0/120**; Android debug **185 tasks PASS**; Capacitor sync nhận plugin Android/iOS; npm audit **0 vulnerability** sau khi loại Firebase Admin. Physical FCM/APNs, credential thật, signed iOS/TestFlight vẫn **NOT CONFIRMED** trên Windows.

---

## Audit QUESTION-BANK-2 — Version, tenant, snapshot and recovery controls — ✅ ENGINEERING VERIFIED (2026-09-02, ADR-096)

- **Threat model:** cross-tenant catalog/blueprint access, non-reviewer publication, mutable historical answer keys, partial blueprint generation, unsupported type coercion, answer leakage through audit, and data loss through old backup/purge contracts.
- **Controls:** composite tenant keys and route/service scope; class-access check; explicit lifecycle; append-only versions; active-only selection; transaction rollback on shortage; exact session snapshots and hashes; ADR-094 manifest reuse; audit contains hashes/counts only.
- **DB fail-closed:** readiness requires migrations `150..158`, new tables/columns/indexes/PKs and same-parish blueprint triggers. OMR materialization rejects anything outside one-answer A–D MC; other unsupported types return typed 422.
- **Recovery:** backup `2.1-question-bank` includes five new table groups; legacy `2.0-production` restore preserves current bank; pre-restore safety snapshot and Purge v2.5/snapshot v3.2 cover the full new delete set.
- **Residual:** coarse staff roles can view answer content as already required by Smart Exam staff preview; granular capability separation requires a later RBAC migration. Authoring is online-only. FTS and production-scale latency are not claimed without measurement.
...
## Audit OPERATIONS-AUTHORITY-1 — Scoped authority and reminder delivery hardening — ✅ FOCUSED VERIFIED / RELEASE GATES OPEN (2026-09-08, ADR-110 amendment)

- **Confirmed findings:** substring title matching could grant Trưởng ngành authority to `Phó trưởng ngành`; organizer/workstream-local manage permission could move a resource to an organizational scope outside the actor's standing authority; mixed event/workstream/task identifiers could combine unrelated operational roles in permission explainability; parent/locked/unlinked targets could be counted as actionable responsibility; targeted Operations reminders did not revalidate current resource authority or terminal resource state at due time; missing queue ownership could be automatically recreated; required cancelled tasks satisfied readiness, rejected approvals could still reach DONE, and terminal task/event structure remained mutable. Event completion also selected an arbitrary Parish Memory row by nonunique `source_event_id` and could overwrite user-owned history. List loops swallowed authorization-query infrastructure failures into apparently valid partial results.
- **Controls:** controlled `parish_service_terms.position_code` with exact compatible legacy backfill, service validation and DB triggers; destination-scope authority plus resource-graph consistency; staff/actionable-target checks; accepted-assignment gate for execute/approve/readiness; READY→LIVE revalidation and terminal lifecycle guards; UTC instant canonicalization; explicit decisions in list filtering so database failures propagate; deterministic reminder IDs, create/due resource reauthorization, terminal-resource suppression, generic bodies and terminal failure for ambiguous missing rows; cancellation reason triggers, cancelled-required blockers and APPROVED-only completion. Post-event history now inserts a distinct `ACTIVITY` transactionally and never updates by soft event link.
- **Tenant/privacy:** every new predicate retains `parish_id`; parent/parent-linked users cannot become Operations actors/recipients; notification revalidation never retargets or broadcasts and the external body contains no internal title. Reminder inbox SQL-pagination returns only user-facing fields and omits dedupe keys, queue/provider diagnostics, recipient IDs and stale resource IDs; the client does not cache it. Existing Parish Memory records are not mutated through a soft association. Position display text remains free-form but has no authorization effect. Frontend route policy remains UX only.
- **Evidence:** latest residual affected boundary **6 files / 56 tests PASS**, including Operations integration **18/18**; latest broader Operations/organization/schema/migration/notification/frontend boundary **9 files / 93 tests PASS**; client/server TypeScript and production build (**2,824 modules; PWA 266 entries**) PASS. Final lint, architecture inventory and Design-System **0/134** pass; the security-critical suite remains **73/73 PASS** from the unchanged boundary checkpoint. Production migration application, provider smoke, receipt-response retention policy and physical device/pilot remain NOT CONFIRMED.

---

## Audit OPERATIONS-RESIDUAL-2 — Pagination, offline read projection and receipt lifecycle — ✅ FOCUSED VERIFIED / POLICY AND FIELD GATES OPEN (2026-09-08, ADR-110 amendment)

- **Confirmed findings:** private Operations lists returned unbounded rows and performed authorization queries per row; the browser API always sends a custom idempotency header but production CORS preflight did not allow it; receipt response snapshots had no safe compaction mechanism; Operations had no durable private read projection. The generic sync queue cannot safely absorb Operations writes yet because it lacks the Operations parent-ID remap, dependent ordering, per-resource OCC reconciliation and conflict UX contract.
- **Controls:** event/workstream/task/reminder lists now return authorized pagination metadata (50 default, cap 500), with actor/resource authorization evaluated from a batch snapshot rather than N+1 queries; reminder inbox additionally performs page/count in SQL and emits a minimal projection. CORS preflight permits both supported idempotency request headers. Encrypted `dexieStorage` retains only server-confirmed event/task overview rows under exact `parishId:userId`; mixed/cross-scope cache is rejected, reminder inbox and permission maps are never cached, only transport status `0` may fall back, and stale state cannot enable mutations. Migration `20260908-234` plus policy-gated maintenance compacts receipt response bodies while retaining immutable key/command/hash tombstones; late retries return `409 IDEMPOTENCY_REPLAY_EXPIRED` and cannot re-execute.
- **Limits:** the retention duration remains disabled until an owner configures `OPERATIONS_RECEIPT_RESPONSE_RETENTION_DAYS`; Web/Native Push delivery remains at-least-once because the provider boundary has no generic transactional idempotency contract. Telegram is retired under ADR-111 and is not an Operations delivery path. Durable Operations writes, representative production/pilot latency, live migration/compaction inventory and physical-device acceptance remain unclaimed.
- **Evidence:** focused boundary **9 files / 93 tests**, security-critical **7 files / 73 tests**, isolated real-backend E2E **1/1**, production build, lint, architecture inventory and Design-System **0/135** all PASS. Synthetic local baseline (250 events, 1,000 tasks, 250 required readiness tasks, page 50, 20 samples) measured p95 event/task/readiness at **13.43/78.12/32.49 ms**; this is regression evidence only, not a field SLO. The current Vitest suite passed in bounded partitions (**334 files / 2,390 tests** plus isolated password-reset **1 file / 6 tests**); the first monolithic invocation exposed a suite-order/resource stall and is not represented as a one-run green gate.

---

## Audit CSP-SRCDOC-PREVIEW — Print/preview pipeline vs production CSP — ✅ FOCUSED VERIFIED / FIELD SMOKE OPEN (2026-09-08)

- **Confirmed findings:** (1) localhost (Vite dev, không CSP header) render `<style>` inline trong iframe `srcDoc` preview bình thường; production (`style-src 'self'` không `unsafe-inline` ở cả `vercel.json`/`nginx.conf`/`security.ts`) chặn toàn bộ `<style>` trong `srcDoc` (srcdoc kế thừa CSP parent) → preview đề thi/phiếu thu mất layout: header grid rớt thành block, QR rớt giữa trang, marker OMR SVG phình full-width thành vệt đen, bubble xẹp. (2) Đính chính nhận định A-NEW-23: tài liệu popup Blob URL do trang mở ra **CÓ kế thừa CSP opener** (verified: console popup nhận violation style-src/script-src) — chỉ blob ở top-level fresh (không opener) mới không kế thừa; hệ quả là inline auto-print script và toàn bộ `<style>` inline trong popup in/PDF cũng bị chặn trên production. (3) `win.onload` gán từ opener bị MẤT khi navigation commit (HTML spec: Document mới = Window mới; verified 5 probes + E2E: load event phát ra nhưng flag opener không set) → auto-print onload-only không bao giờ nổ. (4) Hidden iframe `src=blob:` (exportPdf cũ, print fallback) bị `frame-src 'none'` (Vercel) / `default-src 'self'` (nginx) chặn — navigation fail nhưng `onload` vẫn nổ trên frame rỗng → in ra trang trắng.
- **Controls (KHÔNG nới CSP, giữ `sandbox=""`, giữ `opener=null`):** 5 builder in (`examSheets` ×4 + `receiptGenerator`) nhúng `<link>` stylesheet **tuyệt đối** cùng origin (`withPreviewStylesheet` — tương đối chết vì base URL `blob:` không resolve được trong popup, verified `link.sheet` null); 5 file `public/print-*.css` mirror khối `<style>` (1 doc — 1 file, tránh cascade `@page`/`body`); giá trị động chuyển class tĩnh (`cols-2`, `voucher-*`); `exportPdf` chuyển popup; `print()` fallback iframe → toast; `printQrSheet` null-path → toast; `printReceipt` gộp về `ReportExportService.print`; auto-print chuyển polling `readyState` opener-side (250ms, cap ~3s, guard `printed`) thay inline script + onload-only; precache 5 CSS trong SW (offline-first-open).
- **Privacy/tenancy:** không đụng auth/tenant; filename/title PDF vẫn qua `sanitizeFilename`/`applyPdfTitle` (EP-F1 giữ nguyên, có test evil-filename).
- **Evidence:** Chromium thật với đúng header Vercel — preview `srcDoc` 50 câu: header `grid` 4 cột, lưới OMR `grid` 8 cột, marker `18px`×4, bubble `16px`×200, 2 cột; popup blob: style tương đương + `w.print()` nổ đúng 1 lần (recorder), `opener` null. Unit **12 files / 126 tests PASS** (gồm sync mirror, tripwire CSS-tĩnh, exportPdf popup/blocked/XSS-title, polling fallback, FIN-XSS-1 sau delegation), `examQrRender` Chromium thật **9/9**, client `tsc` sạch, `vite build` pass (precache 266), `oxlint` + DS lint sạch. Mỗi lần mở preview vẫn sinh 1 benign `style-src` report (inline dự phòng cho file tải về/Word/Puppeteer) — đã ghi nhận để lọc khi review log.
- **Limits:** smoke tay trên production sau deploy còn mở (modal đề thi + nút Xuất PDF + popup-blocked); E2E repo không cover luồng exportPdf/hidden-frame (đã lấp bằng unit + harness ngoài repo); nút inline `onclick` trong HTML báo cáo popup (nếu có) vẫn bị `script-src` chặn — ngoài phạm vi đề thi.

---

## Audit OPERATIONS-XUDAN-AUTHORITY — Xứ Đoàn organizational authority model (ADR-112) — ✅ FOCUSED VERIFIED / PROD GATES OPEN (2026-09-11, D3)

- **Scope:** 7 stable `parish_service_terms.position_code` (`PARISH_LEADER|PARISH_SECRETARY|PARISH_DEPUTY ↔ BOARD/toàn xứ`, `BRANCH_LEADER|BRANCH_DEPUTY ↔ BRANCH`, `COMMITTEE_LEADER|COMMITTEE_DEPUTY ↔ COMMITTEE`); `operation_events.event_scope_type` (`XU_DOAN↔scope_unit_id NULL`); creator≠organizer server enforcement; Field = Workstream + mandatory `sourceUnitId` + leader-must-be-Leader; `tasks.scope_unit_id`; `GET /operations/creation-options` as the server-authoritative "+ Tạo mới" source. Migrations `20260912-256` (CHECK rebuild + triggers), `257` (scope type + backfill), `258` (task scope), `259` (exact-match deputy/secretary backfill of NULL codes left by `20260908-230`).
- **Confirmed findings fixed:** technical `users.role` acted as business authority (`ADMIN` ≡ Xứ đoàn trưởng, creator ≡ organizer, any staff assignable as Field lead or cross-unit target); event-bound workstream creation omitted `sourceUnitId`; follow-up tasks omitted scope; `PUT /events` could desync `eventScopeType ↔ scopeUnitId`; template instantiate hardcoded self-organizer (deputy 400 with no UI escape); parish-profile terms UI hardcoded 3 codes (new codes unassignable + render crash on unknown codes); frontend hid PREPARING paths the backend allows.
- **Controls:** O1–O8 locked decisions; ADMIN keeps parish-wide mutation override on all environments except `task.execute`/`task.approve` (O2 — supersedes the older "read-only in production" sentences in ADR-110/AI_CONTEXT_MAP); parish-wide target scope covers ADMIN + `PARISH_LEADER|SECRETARY|DEPUTY`; deputy-created events require a leader organizer; deputy Field leads rejected (`WORKSTREAM_LEAD_OUTSIDE_UNIT`); standalone tasks require scope at route layer (no DB trigger by design — the ADMIN compat path is actor-aware); every mutation keeps Idempotency-Key + OCC + audit + `parish_id` predicate; candidate pickers remain UX-only.
- **Tenant/privacy:** every new predicate retains `parish_id`; organizer display resolution returns names only; no contact/PII added to candidates, reminders, audit or projections.
- **Evidence:** `operations.test.ts` 55/55 (incl. creation-options/scope/organizer/deputy-ban + manager-nudge creator+organizer recipients), related suites 107/107 (readiness/rehearsal marker `20260912-259`/receipt/parishProfile/migration/store/page), `tsc -b` + `oxlint --deny-warnings` + Design-System lint clean, synthetic benchmark re-measured (no consistent regression signal beyond run noise; not a field SLO).
- **Limits:** production migration rehearsal on a finalized backup + `audit:operations-authority` preflight (mandatory before upgrade — migration 259 aborts fail-closed on overlapping same-code terms), Playwright E2E rerun in CI, receipt-retention duration, cross-unit workflow, event-level observer, strict leader-only lead and task-scope NOT NULL remain tracked follow-ups. Pre-existing architecture-inventory drift at HEAD is unrelated to this change.

---

## Audit CSP-TOOLBAR-1 — Vercel Toolbar vs strict production CSP — ✅ REPO FIXED / OPERATOR GATE OPEN (2026-09-23)

- **Symptom (production, browser của owner):** mỗi lần mở `https://tnttvn.vercel.app`, console báo 6× `Applying inline style violates the following Content Security Policy directive 'style-src 'self' https://fonts.googleapis.com https://vercel.live' … The action has been blocked.` với stack `U @ feedback.js:2` (Vercel Toolbar/Feedback script).
- **Root cause — Chromium-verified (không suy diễn):** probe Playwright 1.61/Chromium thật với đúng header CSP production chạy 9 thao tác style → **chỉ việc chèn `<style>` element** (`style.textContent`, `style.innerHTML`, `<style>` trong `iframe srcdoc` — srcdoc kế thừa CSP parent) bị chặn và phát **đúng verbatim** thông báo trên (hash trong probe trùng hash báo cáo thật: `sha256-P7BPa/3lSPZewZRv7P+Ht00YyK8Rw0ud1yDqZnH/Eak=`); `element.style.x`, `style.setProperty`, `style.cssText`, `setAttribute('style')`, `insertRule` trên sheet same-origin, constructed `CSSStyleSheet` + `adoptedStyleSheets` đều PASS nhờ `style-src-attr 'unsafe-inline'`. Kết luận: đây là xung đột cấu trúc — toolbar xây UI bằng `<style>` element nên **không thể** allow bằng host trong `style-src`; chỉ `style-src 'unsafe-inline'` mới thỏa (A-NEW-23 cấm; Vercel staff xác nhận toolbar không hỗ trợ strict CSP).
- **Blast radius:** HTML production ẩn danh KHÔNG chứa `vercel.live` (verified `curl`: chỉ `theme-boot.js` + bundle app) → lỗi chỉ hiện cho viewer đã đăng nhập Vercel. Nhưng mỗi vi phạm còn được browser gửi tới `report-uri /api/csp-report` → log WARN production bị nhiễu đúng kênh dùng để phát hiện XSS (OBS-1); `script-src https://vercel.live` còn cho phép script origin thứ ba chạy trong app origin (memory token + PII) mà không đổi lại chức năng nào.
- **Fix (repo, security boundary):** (1) `vercel.json`: gỡ toàn bộ allowance `vercel.live`/`assets.vercel.com` khỏi `script-src`/`style-src`/`font-src`/`connect-src`/`frame-src` (revert commit `57b778e`); (2) `deploymentSecurityContract.test.ts` thêm helper `cspDirective()` + assert `script-src 'self'`, `style-src 'self' https://fonts.googleapis.com`, `style-src-attr 'unsafe-inline'` và cấm `vercel.live`/`assets.vercel.com`; (3) `deploy-production.yml` smoke fail nếu header CSP production còn `vercel.live`; (4) `DEPLOYMENT_GUIDE` §7.0 + `02_ARCHITECTURE` ghi yêu cầu vận hành (Toolbar Off cho Production, `x-vercel-skip-toolbar: 1` cho automation, lý do KHÔNG được nới `style-src`).
- **Operator gate còn mở:** repository không sửa được Vercel project settings — cho tới khi operator đặt **Settings → General → Vercel Toolbar → Production: Off** (kiểm tra cả team-level), Vercel vẫn inject `feedback.js` và CSP sẽ chặn ở `script-src` (đổi 6 lỗi style thành 1 lỗi load-script). Trạng thái console sạch và smoke không-`vercel.live` chỉ đạt sau bước vận hành này; chưa verify bằng browser đã đăng nhập Vercel sau khi đổi setting.
- **Không ảnh hưởng:** app native (Capacitor không nhận `vercel.json`/toolbar); `nginx.conf` và `server/src/middleware/security.ts` vốn chưa từng allow `vercel.live` → ba lớp policy đồng bộ hơn; UI trong app (React `style={}`, preview `srcdoc` với CSS tĩnh) giữ nguyên vì `style-src-attr`/stylesheet không đổi.
