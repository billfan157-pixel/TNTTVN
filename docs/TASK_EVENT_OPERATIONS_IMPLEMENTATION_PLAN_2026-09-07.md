...
Checkpoint cơ cấu tổ chức 2026-09-09: Ban Điều hành là root và trình bày đủ nhóm chức vụ Trưởng Xứ đoàn, Phó Xứ đoàn 1/2, Thư ký, Thủ quỹ, Ủy viên. Ngành và Ban chuyên môn được sửa thành hai nhánh cấp 2 song song; Trưởng ngành phụ trách giáo lý, Trưởng ban phụ trách chuyên môn Xứ đoàn, một person có thể kiêm nhiệm bằng hai service term. Chỉ ba `position_code` leader hiện hành cấp capability; các chức danh còn lại không bị nhét vào `users.role` và chỉ có quyền vận hành khi được gán role resource cụ thể. Service + migrations `20260909-248..249` bảo vệ cấu trúc writer mới và chặn vô hiệu hóa/đổi loại Board còn child; migration `250` chặn hai nhiệm kỳ leader chồng ngày trong cùng scope nhưng không chặn một person kiêm leader ở scope khác. Legacy root Branch/Committee và leader-term overlap phải được inventory/reconcile có duyệt, không tự backfill/chọn người thắng.

Checkpoint authority Gate B 2026-09-10: runtime đã tách account role khỏi organizational/resource authority, cô lập admin mutation override khỏi production, giới hạn public publish/cancel cho Trưởng Xứ đoàn, bỏ quyền đọc suy diễn từ membership thường, chặn Workstream Lead tự bổ nhiệm lead khác và enforce separation-of-duty theo canonical person. Service-term mang authority cần admin reauth, reason, audit và chặn self-grant. Managed writer và partial unique index migration `20260910-251` chặn BOARD active thứ hai; dữ liệu duplicate làm migration fail-closed. Preflight chỉ đọc `npm run audit:operations-authority` đã chạy trên database local, mặc định băm identifier và phát hiện 6 finding cần quyết định thủ công: thiếu BOARD root (1), bốn COMMITTEE còn ở root (4), thiếu Trưởng Xứ đoàn hiệu lực (1); không tự sửa dữ liệu và không suy ra production. Regression nền đã đạt, nhưng focused Vitest cho delta hardening cuối cần rerun khi runner không còn `spawn EPERM`; build/type/static gates hiện xanh.

Status: Online-first Operations MVP including the approved event lifecycle and primary/reserve dispatch is implemented and locally verified; pilot/environment/release gates remain
Date: 2026-09-07
...
### Điều chỉnh sau nghiên cứu 2026-09-09

[Thiết kế và verification gates P2–P5](OPERATIONS_P2_P5_DESIGN_REVIEW_2026-09-09.md) là phần bổ sung hiện hành cho roadmap. Các checkpoint bên dưới là lịch sử, không dùng các câu “chưa triển khai” cũ để phủ nhận code đã có.

### Checkpoint calendar ownership, task shifts và closure — 2026-09-09

- `/operations` là command surface duy nhất cho event. Form tạo/sửa có toggle `Nội bộ | Công khai`; client không còn chọn hay ghi `sourceParishEventId`.
- `DesktopCalendarView`, `MobileCalendarView`, calendar store và calendar API client là read-only. Server giữ `GET /api/parish-events`; mọi direct POST/PUT/DELETE trả `405 CALENDAR_READ_ONLY`. Bộ lọc ngày/loại sai trả 400, không bị bỏ qua để mở rộng query.
- Public Operations event tự tạo/cập nhật `parish_events` và durable parent notification trong cùng idempotent transaction; internal không tạo projection. Chuyển về internal hoặc cancel soft-delete projection. Calendar response không chứa task/assignee/comment/readiness/hậu kiểm.
- Task có optional cặp `scheduledStartAt/scheduledEndAt`, được bảo vệ bằng schema/check/trigger/index. Assignment/handover/follow-up conflict dùng half-open interval overlap; deadline-only cũ là fallback. Template snapshot/preview/instantiate giữ shift theo offset tương đối.
- Theo quyết định vòng đời 2026-09-10, đánh giá chỉ ghi sau COMPLETED và không còn chặn hoàn thành. Chuyển hoàn thành thủ công vẫn yêu cầu outcome summary và mọi required task DONE; worker đến giờ kết thúc được phép hoàn thành với ghi chú số việc chưa xong mà không tự sửa task/checklist. PREPARING, chuyển tiến/lùi, pause/resume automation, tự LIVE/COMPLETED và người chính/dự bị đã nối runtime; xem `OPERATIONS_EVENT_LIFECYCLE_REVIEW_2026-09-10.md`.
- Verification mới nhất cho lát cắt dispatch: backend 52/52 và frontend/store 49/49 đạt; server build và frontend `tsc -b` đạt. Gate tổng hợp migration/schema/lifecycle/component phải tiếp tục xanh trước bàn giao. E2E mới cho primary/reserve và field gates vẫn còn mở.

- P2: phase backend/type/UI và E2E đủ ba giai đoạn đã có. Checklist kế thừa phase của task, không thêm phase riêng. Required task đã hủy không được miễn closure; command khôi phục riêng `CANCELLED -> TODO` yêu cầu task.manage, reason, OCC, receipt/audit và không phục sinh reminder đã terminal.
- P3: reminder version, resource list, reschedule/cancel OCC, worker due-time recheck và UI đã có. Regression race dùng điểm đồng bộ xác định; E2E create→reschedule→reload→recipient cancel đã đạt.
- P4: hoàn tất local. Scoped task read, candidate/unit directory tối thiểu, membership validity OCC, self blockout lifecycle, bàn giao Trưởng nhóm khi LIVE và nhóm độc lập có organizational scope đã có. Trưởng ngành/Trưởng ban bị giới hạn cả read picker lẫn write command vào active service-term của unit/descendants; Admin/Trưởng Xứ đoàn giữ phạm vi toàn xứ đoàn. E2E nhóm độc lập → task → cảnh báo blockout riêng tư → giao/nhận đạt Chromium và WebKit. Deadline không thay thế khoảng thời gian phục vụ.
- P5: structured retrospective/follow-up và hạ tầng template versioned/lifecycle đã hoàn tất local. Row hậu kiểm 1:1 có OCC; follow-up command tạo task phase FOLLOW_UP + OWNER PENDING + hạn trong một transaction và revalidate phạm vi Ban/Ngành. Template snapshot không copy người nhận, acceptance, approval result, comment, dependency/workstream hay reminder; preview dùng exact version và deadline/shift tương đối, instantiate nguyên tử thành event DRAFT/task TODO/checklist chưa xong. Public instance dùng cùng server-owned projection path, không tạo calendar source thứ hai. Archive/restore template là metadata lifecycle có reason, scope authority, monotonic family version + expected latest content version, status CAS, receipt/audit; không xóa snapshot hoặc provenance. Chỉ tập mẫu nội dung curated còn deferred tới pilot/duyệt nghiệp vụ.

E2E P2/P3/P4, structured-retrospective P5 và template lifecycle P5 đạt Chromium và WebKit trên sandbox local. Journey template chứng minh snapshot → preview → instantiate không kế thừa assignee → archive → restore và API/DB read-back. Local checks không thay thế production migration, phục hồi backup, multi-instance remote, pilot nội dung hoặc kiểm chứng thiết bị/push thật.

### Checkpoint P2/P3 — 2026-09-09

P2 frontend dùng phase bắt buộc trong contract, picker khi tạo và nhãn trước/trong/sau ở danh sách/chi tiết. E2E thật chứng minh PREPARATION chặn start, EXECUTION/FOLLOW_UP chỉ cần OWNER nhận trước start và cùng chặn closure cho tới DONE.

P3 migration `20260909-237` backfill reminder version 1. Reschedule/cancel dùng expectedVersion, receipt và CAS; resource list authorize trước pagination. Worker đọc lại due-time trong transaction và mọi transition lịch gửi tăng version; retry catch không ghi đè reminder đã dời. UI cho tải, đổi giờ/hủy với lý do và giữ draft khi lỗi. Backend/migration/schema 52 tests đạt; frontend Operations/reminder 43 tests đạt; server build, frontend build, lint và Design System đạt. E2E P2 và P3 mỗi hành trình đạt trên Chromium/WebKit. Đây là bằng chứng LAB/sandbox, không phải field/production certification.

P4 đã có read slice `workstreamId`, `GET /candidates` contact-free theo đúng một resource, `GET /units` chỉ trả scope có quyền tạo nhóm, validity editor, self blockout lifecycle, command bàn giao Trưởng nhóm LIVE và panel nhóm độc lập. Operations UI không còn tải toàn bộ Parish Profile để dựng picker phân công/nhóm/nhắc/bàn giao; hook, lịch bận và panel mới loại response đến sau khi đổi tenant/account. Trưởng ngành/Trưởng ban chỉ nhận và chỉ ghi target có nhiệm kỳ hiệu lực trong unit/descendants; guard chạy lại trong transaction cho assign/handover/workstream member/lead replacement. Nhóm không event bắt buộc `sourceUnitId`; API/UI hỗ trợ tạo nhóm → thêm member → tạo task → giao/nhận. Admin/Trưởng Xứ đoàn vẫn parish-wide. Membership validity chỉ đổi start/end với reason, receipt, audit và CAS kép; role/target không bị sửa ngược lịch sử. Khi event LIVE, add/remove/validity rời rạc của lead bị khóa; command hẹp soft-remove lead cũ, insert lead actionable mới và CAS workstream trong cùng transaction, hoặc bổ nhiệm lead đầu tiên cho nhóm phát sinh chưa có lead. Migration 238 thêm blockout version; self list/edit/revoke giữ reason riêng tư, manager warning/audit không phát reason. Regression hiện tại: 35 Operations backend tests, migration/schema gates, 3 candidate-hook tests và 58 component/store tests liên quan đạt; lint, Design System và server build đạt. E2E P4 thật đạt Chromium 19,2 giây và WebKit 48,4 giây với DB sandbox/read-back; P4 được xem là hoàn tất local, không thay thế production/device certification.

P5 structured retrospective/follow-up checkpoint: migration `20260909-239`, schema health và detail projection đã có. PUT retrospective dùng expectedVersion/receipt, chỉ sau COMPLETED và audit metadata không chứa nội dung. POST follow-up bắt buộc dueAt, actionable target và đúng scope tổ chức; CAS event rồi tạo task FOLLOW_UP + OWNER PENDING trong cùng transaction. UI giữ outcome đã chốt ở chế độ đọc, chỉ hiện candidate actionable, chặn double-submit đồng bộ, loại response sau đổi session và không lộ blockout reason. E2E retrospective đạt Chromium/WebKit với recipient acknowledgement và API read-back. TypeScript hiện đã xanh trên current worktree; số liệu từng checkpoint cũ bên dưới vẫn là lịch sử theo snapshot tương ứng.

Checkpoint restore/template 2026-09-09: `POST /tasks/:id/restore` đóng dead-end của required cancelled task mà không biến cancel thành done hay mở closure override; lifecycle note/start/block/complete được reset và lịch sử nằm ở audit. Migrations `20260909-240..243` thêm template family/version bất biến, provenance event có trigger same-parish/version/delete protection và family `version` monotonic chống ABA cho lifecycle. API list/preview/snapshot/version/instantiate/archive/restore authorize theo scope trước pagination/read, dùng OCC/status CAS + receipt/audit và transaction all-or-nothing. UI tách catalog instantiate ở tiện ích chính khỏi snapshot/version/lifecycle trong modal event, bắt buộc preview latest, giữ calendar link explicit, công bố rõ không copy nhân sự/trạng thái quyền và refresh catalog sau mutation. Projection template được gắn với exact `parishId:userId`, ẩn ngay khi đổi scope và bỏ response tenant cũ về muộn. Final affected-boundary regression đạt 84/84 ở Operations/migration/schema/restore/template/page; full lint, Design System, architecture inventory và production build đạt. E2E template lifecycle UI→API→DB đạt Chromium 16,1 giây và WebKit 43,2 giây. Local SQLite migration/recovery harness sau đó đạt 3/3 regression: từ chối DB live, fail trước migration với FK violation, và xác nhận copy→migrate→schema/integrity→recovery không đổi dữ liệu Operations. Curated templates, migration trên backup production/Turso, recovery thực địa và pilot vẫn chưa được chứng nhận.

Checkpoint authority/date 2026-09-09: hai phép lọc nhiệm kỳ Operations trước đây lấy ngày UTC, nên có thể cấp/thu sai quyền Trưởng ngành/Trưởng ban và sai phạm vi người được giao việc trong 7 giờ đầu ngày Việt Nam tại biên nhiệm kỳ. `server/src/utils/parishTimeZone.ts` nay tạo ngày dân sự server-authoritative từ `PARISH_TIME_ZONE` (mặc định `Asia/Ho_Chi_Minh`); cả actor resolver và target-scope guard dùng cùng helper, cấu hình IANA sai làm startup fail. Regression cố định đúng instant chuyển ngày đạt cùng unit allow/deny. Event timezone trước đây cũng chỉ kiểm chuỗi và completion cắt ngày UTC khi tạo Parish Memory; API nay validate IANA và `occurred_on/ended_on` được materialize theo timezone của event, với regression sự kiện 00:30 giờ Việt Nam. Không thêm timezone vào user/browser hoặc bảng tenant vì production topology hiện là một deployment/một giáo xứ; nếu mở multi-timezone tenancy phải có ADR riêng.

Archive/restore template metadata đã hoàn tất theo policy hẹp: `is_active` được đổi bằng command có thể đảo ngược, reason, current scope authority, expected monotonic family version + latest content version, status CAS, receipt và audit. Family version tăng ở create-version/archive/restore nên command cũ không thể hợp lệ lại sau vòng archive→restore (ABA). Mẫu archive bị loại khỏi catalog/preview/version/instantiate; immutable content version và event provenance tiếp tục được giữ.

### P2 checkpoint — phase backend

Verification hiện tại: migrationRunner + schemaHealth + Operations 50/50 tests đạt; backend TypeScript build và lint đạt. Fixture schema đầy đủ đã được sửa để phản ánh cột mới, không bỏ schema guard. Hai regression EXECUTION/FOLLOW_UP chứng minh owner pending chặn start, accepted cho start, chưa DONE chặn closure và DONE cho closure.

Migration 20260909-236 thêm phase có CHECK enum và DEFAULT PREPARATION, không sửa status dữ liệu cũ. Schema health yêu cầu cột/migration. API create nhận phase; start readiness phân biệt chuẩn bị với EXECUTION/FOLLOW_UP, closure kiểm tra mọi required task. UI hiện đã chọn/hiển thị phase nhưng chưa hỗ trợ sửa phase sau tạo. Rollout phải nâng toàn bộ writer backend trước khi tạo phase khác PREPARATION; không chạy song song writer cũ, không rollback bằng drop cột. Rollback UI và forward-fix giữ dữ liệu. Chưa triển khai production; còn gate migration/recovery và chính sách required task đã hủy trước khi đóng P2 hoàn toàn.
...
**Trạng thái hiện tại đã hợp nhất:** event/task/workstream core, task detail độc lập, approval/comment/bàn giao, warning bận user/person→UI, nhắc event/task, cancel/reschedule có OCC, membership validity, nhóm độc lập, P2 phase/closure, P5 retrospective/follow-up và template version/lifecycle đều đã có code và verification local tương ứng. Các đoạn “continuation/checkpoint” bên dưới là lịch sử từng lần triển khai; câu cũ về capability chưa hoàn tất không còn mô tả snapshot hiện tại. Không dùng status đầu tài liệu để kết luận production migration, provider/device delivery, pilot hoặc release đã được chứng nhận.
...
6. P5: tổng kết, follow-up và hạ tầng mẫu versioned/lifecycle đã hoàn tất local; pilot chỉ còn quyết định nội dung mẫu curated. Chưa thêm recurrence engine.
...
- **Deferred tại checkpoint 2026-09-08:** generic grants/workflow, template infrastructure, recurrence, delegate/snooze, budget/finance, RSVP/public registration, attachment upload and durable offline Operations **mutations**. Template infrastructure was subsequently delivered on 2026-09-09; only curated template content remains pilot/approval-gated. The other items still lack approved conflict/remap/business semantics.
...
Historical rescheduling research (superseded by the current P3 checkpoint): reminders originally had no monotonic version. An expected-triggerAt-only check would miss an ABA change (time changed away then back), so no such partial OCC contract was introduced. The delivered implementation instead added migration-backed versioning for create/reschedule/cancel/enqueue transitions with concurrent scheduler tests.
...
- Checklist trực tiếp theo task; template snapshot hiện sao chép checklist của task nhưng không sao chép trạng thái hoàn tất. Chỉ thư viện nội dung curated theo loại sự kiện còn chờ pilot.
...
- `POST /api/operations/workstreams/:id/members`, `/members/:memberId/remove`, `/lead/replace`
...
- **Đã chốt timezone:** dữ liệu operation dùng UTC ISO instant + IANA timezone hợp lệ của event; Parish Memory date được materialize theo timezone event. `parish_service_terms` date-only dùng `PARISH_TIME_ZONE` do server/deployment sở hữu (mặc định `Asia/Ho_Chi_Minh`), không dùng ngày UTC hay browser. Dữ liệu lịch cũ `date` + `time` giữ nguyên contract và chỉ chuyển đổi tại boundary.
...
- Event detail + task checklist + readiness. Template instantiation exact-version được hoàn tất ở P5, giữ `parish_events` là nguồn lịch công khai duy nhất.
...
- Complete event và outcome; hậu kiểm có cấu trúc lưu riêng và follow-up là task phase FOLLOW_UP được tạo cùng một OWNER/hạn bằng command nguyên tử, không thêm workflow tự động.
- E2E critical path: create event -> assign -> acknowledge -> complete -> ready/live/completed.

Checkpoint: **IMPLEMENTED / LOCALLY VERIFIED**. Responsive inbox/action flow, direct checklist, lifecycle completion, outcome summary, idempotent `parish_records` activity projection, retrospective OCC và atomic follow-up OWNER đã có. Real-backend E2E đi qua UI để chứng minh create event -> create/check task -> assign -> acknowledge/complete -> PLANNING/READY/LIVE/COMPLETED; E2E P5 riêng chứng minh save hậu kiểm -> tạo follow-up -> recipient nhận việc -> API read-back trên Chromium/WebKit.

### Slice 5: Phase 2 extensions

- Recurrence, RSVP/session, exports và template automation nâng cao. Template snapshot/version/preview/instantiate/archive/restore cơ bản đã hoàn tất; dependency DAG tối thiểu và explicit approval role đã được kéo vào task core vì readiness/completion hiện tại phụ thuộc trực tiếp vào chúng.
...
- **M5 — Đúng, MEDIUM; đã đóng local:** calendar hiện dùng `YYYY-MM-DD` + time text như `HH:MM`; Operations đã chốt instant/timezone boundary, event IANA validation, Parish Memory local date và deployment parish timezone cho service-term authority. Production vẫn phải review exact env/config và dữ liệu hiện hữu.
...
- Operations-owned files pass targeted whitespace inspection. Latest full-worktree `git diff --check` is currently non-green only because concurrent out-of-scope `src/components/parish/ParishLogoModal.tsx` and its component test have a blank line at EOF; they are preserved rather than silently rewriting another change. Manual device/accessibility pilot remains a release/pilot gate and is not claimed from local automation.

- Continuation 2026-09-09: current Operations boundary passed **8 files / 92 tests across two bounded invocations**, including migration/schema/recovery harness, server-authoritative service-term date, invalid-IANA rejection, midnight-local Parish Memory date and template/restore/page regressions. Server TypeScript build, full oxlint, architecture inventory (**32 routes / 6 repositories / 54 services / 12 domain files / 72 tables**) and scoped diff check passed. Frontend production build/Design System were not rerun because this continuation changed only backend/config/docs and the already-green Operations components were rerun directly.

Residual risks sau current hardening: retention **duration** chưa được governance phê duyệt nên maintenance mặc định tắt; event/workstream/task list vẫn phải materialize candidate resource graphs trước khi batch-authorize/slice và cần production/pilot telemetry để đặt SLO (reminder inbox đã SQL-page/count); event detail/readiness aggregate mới có synthetic local baseline, chưa có field/soak evidence; durable Operations writes, provider-level exactly-once và manual mobile/device acceptance vẫn deliberately deferred. Dead endpoint không còn gây retry trùng, nhưng transient partial provider failure hoặc crash sau acceptance vẫn cần per-endpoint ledger nếu sản phẩm yêu cầu exactly-once. Parish-local service-term authority, event IANA validation và Parish Memory local-date materialization đã đóng local; deployment vẫn phải review `PARISH_TIME_ZONE` và inventory dữ liệu event cũ trước rollout. Recurrence/RSVP/generic grants và curated template content vẫn decision/pilot-blocked, không phải thiếu code trong current approved MVP. Controlled `position_code` đã đóng rủi ro suy quyền từ title; `position_title` chỉ còn là nhãn hiển thị. Workstream/member manager UI, validity range, nhóm độc lập, reminder authoring và template lifecycle đã triển khai trong current worktree. SQLite rehearsal harness đã có nhưng chưa được chạy trên backup production và không thay Turso/R2 drill. Suite-order/resource interaction của password-reset test cần được theo dõi ở CI nhưng isolated behavior đã xanh và không có evidence nối nó với Operations.
