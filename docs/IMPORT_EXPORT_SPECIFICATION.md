...
### 7.5 Import theo phần — ô import theo hình thức (UI-POLISH 2026-08-25)
- Modal "Tạo Phiên Chấm": chọn **Hình thức trước**, ô import hiển theo hình thức — `mixed` (Kết hợp TN + TL): **2 ô riêng** "Phần Trắc Nghiệm" + "Phần Tự Luận"; `multiple_choice`: chỉ ô Trắc Nghiệm; `written` (tô điểm 0-10): không có ô import. Đổi hình thức tự dọn phần không còn phù hợp (`handleExamTypeChange`). Mỗi ô mở `ExamImportModal` với prop `scope = 'multiple_choice' | 'essay'`; vẫn còn đường import đề gộp qua scope `'both'` nếu cần tái sử dụng.
