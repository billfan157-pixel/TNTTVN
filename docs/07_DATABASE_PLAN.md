...
| 30 | `exam_results` | Per-student score rows for an exam session (scan source `qr_scan/omr/quick_entry`; Phase 4: `answers` JSON map câu → đáp án) — migrations `20260808-084` + `087` (thêm `parish_id` scope multi-tenant + purge) + `092` (answers) | `idx_exam_results_unique` `(exam_session_id, student_id)` UNIQUE, `idx_exam_results_lookup` `(parish_id, exam_session_id)` |
