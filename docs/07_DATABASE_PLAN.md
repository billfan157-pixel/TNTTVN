...
| 29 | `exam_sessions` | Smart Exam Grading session; `answer_key` là key legacy/mã A, `answer_variants` nullable JSON map A–H (ADR-050); idempotency key NOT NULL | migrations cũ + `20260818-124` (`answer_variants`); indexes giữ nguyên |
| 30 | `exam_results` | Per-student score; `answers`, `scan_metadata` aggregate không ảnh, `exam_version` A–H default A. MC scan được server tính lại bằng key theo version | migrations cũ + `20260818-123` (`scan_metadata`) + `20260818-125` (`exam_version`); unique `(exam_session_id, student_id)`, lookup tenant/session |
