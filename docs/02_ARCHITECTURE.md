...
- **Scan Engine v2 trust boundary (ADR-049)** — Camera/QR/OMR vẫn xử lý on-device và hoạt động offline; form protocol T2 ràng buộc identity + template + số câu bằng checksum corruption-detection. Client chỉ đề xuất answers/score và bắt buộc review multi/weak marks. Khi đồng bộ MC source `omr|qr_scan`, `examService` là authority: validate answers rồi tính lại score từ session answer key/questionCount/maxScore trước khi ghi. `exam_results.scan_metadata` chỉ lưu aggregate diagnostics, API cấm ảnh/base64 đệ quy; score adjustments được kéo lại vào store sau offline sync. Stable guided flow là mặc định; live batch không được nâng thành mặc định trước benchmark gate.
