...
- Với mutation nghiệp vụ bắt buộc truy vết, domain row và audit metadata phải commit/rollback cùng nhau. Push/Telegram là side effect sau commit và được retry/báo riêng; lỗi delivery không được khiến client tưởng mutation chưa commit. Audit không sao chép nội dung thông báo, lý do/người duyệt đơn nghỉ hoặc ghi chú điểm danh.
...
4. **Server authoritative**: với phiên trắc nghiệm và source `omr|qr_scan`, server bắt buộc parse/validate `answers`, lấy key của đúng `examVersion` từ `answerVariants`, `questionCount`, `maxScore` rồi tự tính lại `correct/totalQuestions × maxScore` (làm tròn 0,1). `answerKey` legacy là mã A. Mã chưa cấu hình bị từ chối. Điểm client chỉ là proposal; sai lệch được trả trong `adjustments` và ghi audit. Source `quick_entry` tiếp tục dùng score đã validate.
5. **Dấu vết & riêng tư**: `scan_metadata` chỉ chứa version engine/protocol, template, số câu, mã đề, checksum, trạng thái, câu đã sửa và số đo chất lượng tổng hợp. API cấm mọi key ảnh/frame/photo/blob/base64 ở mọi cấp và cấm data URL. Ảnh rà soát chỉ được lưu khi người dùng opt-in, trên thiết bị hiện tại bằng Dexie tenant-scoped mã hóa AES-GCM, tối đa 24 giờ, không upload và có thể xóa ngay.
...
14. **Quét liên tiếp có giám sát (ADR-067)**: chỉ hoạt động khi người chấm bật `Quét liên tiếp`; mỗi proposal vẫn phải bấm Save. Save phải ghi encrypted durable queue theo từng `sessionId + studentId` trước khi camera re-arm; network acknowledgement không được chặn capture. `complete` chỉ flush sau toàn bộ save/remove đã xếp trước.
...
16. **Idempotency kết quả**: client gửi `clientMutationId` ổn định qua retry. Server scope receipt theo `(parish_id,user_id,client_mutation_id)`, lưu request hash + acknowledgement trong cùng transaction với result; cùng hash trả `duplicate` mà không rewrite/audit lại, cùng key khác payload trả 409 `IDEMPOTENCY_CONFLICT`. Server vẫn tự chấm MC/mixed và response itemized phải reconcile điểm authority vào ledger.
...
6. Sinh đề là một transaction server-authoritative bao trọn kiểm tra lớp/phân công, đọc blueprint + current ACTIVE versions, recent-use selection, tạo `exam_sessions`, snapshot nguồn, answer key, 1–8 manifest và audit rồi mới commit. Không được kiểm quyền hoặc chọn câu bên ngoài transaction làm nguồn quyết định. OMR/scoring/finalization tiếp tục dùng authority ADR-051/094, không đọc đáp án hiện tại từ Question Bank.
