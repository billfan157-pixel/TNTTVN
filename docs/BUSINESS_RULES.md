...
- **UI/API canonical (ADR-090/092):** quản lý lớp nằm trong tab `Lớp Học` của `/students`; tab chỉ render cho admin. `/classes` vẫn admin-only để tương thích deep link. Chỉ staff `admin|chunhiem|phuta` được gọi `GET /api/students` và `GET /api/students/:id`; GLV được xem roster/metadata lớp toàn giáo xứ nhưng không nhận danh sách GLV được phân công cho lớp khác. `phuhuynh` phải nhận 403 ở hai endpoint staff và chỉ đọc con qua `/api/parents/my-children`. Ghi hồ sơ thiếu nhi vẫn chỉ trong lớp được phân công; create/update/delete lớp và assignment vẫn admin-only ở server.
...
1. Câu hỏi thuộc đúng một `parish_id`; `branch_id` nếu có phải cùng giáo xứ. Cấp/lớp giáo lý, sách, chương, bài, chủ đề, độ khó và tag là metadata phiên bản, không được suy thành quan hệ lớp/năm học khi chưa có taxonomy chuẩn.
...
6. Sinh đề là transaction server-authoritative: tạo `exam_sessions` tương thích Smart Exam, snapshot nguồn, answer key, 1–8 manifest bất biến rồi mới commit. OMR/scoring/finalization tiếp tục dùng authority ADR-051/094, không đọc đáp án hiện tại từ Question Bank.
