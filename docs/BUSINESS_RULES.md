...
- Mọi bề mặt báo cáo/in/xuất học vụ chính thức của staff và parent phải dùng DTO từ boundary này. Open year dùng current server facts; `FINALIZED|PROMOTED|ARCHIVED` dùng finalization policy + frozen class/cohort/report snapshot. Client store, Dexie, current class hoặc current settings không được làm fallback; thiếu evidence lịch sử phải fail closed (ADR-113).
...
Tạo năm trống qua `POST /api/classes/academic-years` là online-only: client không có durable offline command contract, không được coi việc chọn `currentYear` là tạo năm và chỉ cập nhật inventory/selection hoặc hiển thị thành công sau khi server trả row hợp lệ (ADR-113).
...
### 9.1 Phạm vi
- **DELETE trực tiếp (31 bảng nghiệp vụ)**: `password_reset_requests`, `feedback_messages`, `students`, `grades`, `attendance`, `notices`, `academic_years`, `classes`, `catechist_assignments`, `notifications`, `import_batches`, `import_batch_students`, `grade_import_hashes`, `service_assignments`, `mapping_memory`, `grade_overrides`, `outbox_messages`, `semester_locks`, `academic_year_snapshots`, `promotion_records`, `attendance_sessions`, `assessments`, `exam_result_mutations`, `exam_results`, `exam_sessions`, `refresh_tokens`, `exam_question_snapshots`, `exam_blueprint_rules`, `exam_blueprints`, `question_bank_versions`, `question_bank_items`. Receipt phải xóa trước `exam_results/exam_sessions`; ticket reset xóa trước các quan hệ user được giữ. (SSOT: `PURGE_TABLES` — `server/src/services/purgeService.ts`.)
- **Safety coverage cho xóa dây chuyền**: thêm `assessment_entries`, `exam_finalizations`, `exam_finalization_items`, `leave_requests`, `student_fee_records` vào snapshot, không thêm DELETE trực tiếp. Assessment thủ công không gắn exam session vẫn giữ FK `RESTRICT`: nếu chặn purge thì rollback, không tự xóa ledger để vượt rào bảo vệ.
...
- Trước khi xóa: snapshot v3.3 gồm 36 bảng theo `PURGE_SNAPSHOT_TABLES` (31 bảng DELETE trực tiếp + 5 bảng cascade), scope theo parish, lấy nhất quán trong một transaction. Payload giữ SHA256 checksum và được ghi qua `blobStorage` (R2 khi cấu hình, local safety directory khi fallback); không giữ transaction DB trong lúc ghi blob.
- Sau khi ghi snapshot, transaction xóa re-auth admin rồi đối chiếu digest nội dung toàn bộ safety profile với dữ liệu hiện tại, không chỉ count/version và không phụ thuộc thứ tự SELECT. Khác biệt → `409 SAFETY_SNAPSHOT_STALE`, không DELETE, không tăng `purge_version`, không ghi success audit; lần thử mới phải tạo snapshot mới. Partial JSON restore dùng cùng cơ chế với safety profile 17 bảng và vẫn giữ lifecycle/dependency preflight.
- Toàn bộ DELETE và kiểm tra từng bảng DELETE trực tiếp về 0 nằm trong **cùng transaction** (FK order con→cha + `PRAGMA defer_foreign_keys`); lỗi → rollback. `purge_version` và success audit chỉ commit cùng dữ liệu.
...
- Khi chuyển sang `LOCKED`, tăng `tokenVersion` và thu hồi mọi refresh session trong cùng transaction với counter/audit. Khóa thủ công cũng thu hồi phiên; mở khóa chỉ cho phép đăng nhập mới, không phục hồi access/refresh credential có trước khóa.
- Reset về `ACTIVE` bởi admin; `failedAttempts` xóa khi đổi mật khẩu thành công.
- Login sai với username không tồn tại, tài khoản khóa/ngừng hoạt động, bcrypt legacy cost 10 và cost hiện hành 12 được cân bằng lượng công việc bcrypt (cost 10 + cost 12). Login đúng với hash legacy vẫn nâng lên cost 12; không hạ độ mạnh hash. Đây không phải cam kết thời gian phản hồi tuyệt đối bằng nhau ở mọi môi trường.
...
- Reuse bao gồm request thua atomic claim khi refresh đồng thời; successor của request thắng cũng mất hiệu lực. Trước khi rotate trong transaction, server kiểm lại trạng thái/role/version hiện hành. Client dùng mutex trong tab và Web Locks giữa các tab cùng origin khi có hỗ trợ; client không phối hợp được vẫn chịu chính sách server fail-closed.
...
- **Logout theo thiết bị**: cookie `parish_refresh` (hoặc body token legacy nếu không có cookie) cho phép thu hồi đúng phiên và chuỗi successor đã rotate của nó; không cần access token còn hạn. Không thay đổi `tokenVersion` hay phiên thiết bị khác. Thiếu refresh credential chỉ xóa cookie, không tự chuyển thành logout toàn bộ; force-logout là command riêng. Access token đã phát hành vẫn chịu TTL/kiểm tra live account hiện hành.
- Client xóa token/marker và trạng thái xác thực ngay; chờ xóa snapshot mã hóa theo scope của user cũ trước khi bỏ scope và reset stores. Kết quả logout phân biệt xác nhận server với kết quả dọn snapshot; lỗi kết nối hoặc lưu trữ phải báo rõ và không phục hồi xác thực cục bộ.
...
- Với mutation nghiệp vụ bắt buộc truy vết, domain row và audit metadata phải commit/rollback cùng nhau. `oldValue` dùng làm bằng chứng trước mutation phải được đọc bằng chính transaction/executor của command, không lấy từ global read trước transaction; nếu không còn row active thì command không được ghi một audit snapshot giả. Fingerprint create-intent dùng cho replay chỉ lưu hash một chiều trong audit CREATE, không sao chép payload plaintext. Web/Native Push là side effect sau commit hoặc durable handoff cùng transaction theo từng domain; lỗi delivery không được khiến client tưởng mutation chưa commit. Audit không sao chép nội dung thông báo, lý do/người duyệt đơn nghỉ hoặc ghi chú điểm danh.
...
4. **Hợp đồng điểm (server-authoritative)**:
   - Phần TN của đề mixed tự chấm theo trọng số từng câu **trong manifest của đúng mã đề A–H**: `mcEarned = Σ points(câu đúng)`. Không lấy điểm theo thứ tự câu gốc sau khi đảo đề; không dùng tỉ lệ `correct/count × maxScore` của đề thuần TN kiểu cũ.
...
6. Sinh đề là một transaction server-authoritative bao trọn kiểm tra lớp/phân công, đọc blueprint + current ACTIVE versions, recent-use selection, tạo `exam_sessions`, snapshot nguồn, answer key, 1–8 manifest và audit rồi mới commit. Mỗi lệnh có `buildCommandId` + request hash; retry cùng command/payload trả đúng session đã commit, còn tái dùng command cho payload khác phải conflict. Khi caller không chỉ định seed, seed được derive ổn định từ command. Không được kiểm quyền hoặc chọn câu bên ngoài transaction làm nguồn quyết định. OMR/scoring/finalization tiếp tục dùng authority ADR-051/094, không đọc đáp án hiện tại từ Question Bank.
   Đề thuần trắc nghiệm sinh từ blueprint chấm theo `Σ points(câu đúng)` của snapshot/manifest đúng mã đề; điểm từng câu đã duyệt trong blueprint phải có hiệu lực. Đây là ngoại lệ được duyệt cho đề blueprint: đề thuần trắc nghiệm tạo theo luồng cũ tiếp tục chấm `correct/count × maxScore`, không đổi điểm lịch sử. Lớp đã soft-delete không còn là đích tạo đề hợp lệ.
...
1. `operation_events` là command aggregate và nơi duy nhất tạo/sửa/hủy sự kiện. `parish_events` là read projection công khai do Operations quản lý, không phải writer/source nghiệp vụ độc lập. DRAFT chỉ creator và technical admin cùng giáo xứ được đọc, kể cả đã chỉ định organizer/assignee; DRAFT không có calendar projection hay notification. Khi vào PLANNING, `PUBLIC_SUMMARY` có đúng một projection cùng giáo xứ do server sinh còn `INTERNAL` không có projection. Client không được chọn/sửa `sourceParishEventId`. Lùi về DRAFT gỡ projection nhưng không tuyên bố thu hồi notification đã gửi. Không được làm lộ task, assignee, comment, readiness, hậu kiểm hoặc audit qua API lịch.
...
18. Target Authorization Model (được chủ sản phẩm duyệt): thay thế các câu mâu thuẫn ở rule 11 về phạm vi toàn xứ của Trưởng/Phó Xứ đoàn. Trưởng Xứ đoàn: View toàn xứ (kể cả DRAFT Xứ đoàn để giám sát); chỉ tạo event Xứ đoàn (`event.create` với `scopeUnitId NULL`) + task độc lập (standalone `task.create` không event/workstream) + `publish_public`; được sửa nội dung event Xứ đoàn (manage/transition/cancel, `workstream.manage`/`mark_ready`) kể cả event do Phó/Thư ký/admin tạo; được **bổ nhiệm Field Lead** cho Mảng của event Xứ đoàn (U-20 2026-09-21: ngoại lệ delegate duy nhất, chỉ với Mảng có `sourceUnitId`, target vẫn phải là Trưởng Ban/Ngành đương nhiệm); **không tạo/sửa task bên trong Mảng** vì task tầng Mảng thuộc Ban/Ngành sở hữu Mảng — `EVENT_CREATOR`/`EVENT_ORGANIZER` không phải đường vòng; KHÔNG delegate người khác (assign/reassign) qua position scope, không gồm execute, không tạo event chuyên môn. Phó/Thư ký Xứ đoàn: View toàn xứ; chỉ tạo Event Xứ đoàn; manage chỉ qua resource role; không delegate; cũng không tạo/sửa task trong Mảng (U-20). Trưởng Ban/Ngành: đủ quyền trong unit (view/create/manage + delegate trong unit/descendants) và là tầng sở hữu Mảng. Phó Ban/Ngành: giữ create/manage theo policy hiện tại, thêm `assign`/`reassign` trong unit/descendants (U-20) nhưng vẫn KHÔNG `assign_lead`, không làm Field Lead và không delegate ngoài unit. Event Creator/Organizer/Workstream Lead: manage nội dung nhưng không tự động delegate; Workstream Lead chỉ delegate khi đồng thời còn là Trưởng unit hiện hành. Task Assignee chưa ACCEPTED chỉ view; ACCEPTED được execute/comment. Task trong event Xứ đoàn bắt buộc thuộc Mảng (`TASK_WORKSTREAM_REQUIRED`, trừ admin tương thích fixture cũ và task cũ grandfather); task thuộc Mảng của event Xứ đoàn chỉ Trưởng/Phó Ban-Ngành của đúng đơn vị sở hữu Mảng hoặc `WORKSTREAM_LEAD` của chính Mảng đó được tạo/sửa (`task.create`/`task.manage`), admin override giữ nguyên; Field Xứ đoàn bắt buộc `sourceUnitId` giữ nguyên. Picker là UX, server re-check target-scope trong transaction (`TARGET_OUTSIDE_ORGANIZATION_SCOPE`); candidate parish-wide chỉ còn admin override.
