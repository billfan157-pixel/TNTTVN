...
4. **Server authoritative**: với phiên trắc nghiệm và source `omr|qr_scan`, server bắt buộc parse/validate `answers`, lấy `answerKey`, `questionCount`, `maxScore` của session rồi tự tính lại `correct/totalQuestions × maxScore` (làm tròn 0,1). Điểm client chỉ là proposal; sai lệch được trả trong `adjustments` và ghi audit. Source `quick_entry` là đường nhập tay có chủ ý nên tiếp tục dùng score đã validate. Phiên tự luận/score-grid chưa có đáp án cấu trúc nên giữ score scan đã validate.
5. **Dấu vết & riêng tư**: `scan_metadata` chỉ chứa version engine/protocol, template, số câu, checksum, trạng thái, câu đã sửa và số đo chất lượng tổng hợp. API cấm mọi key ảnh/frame/photo/blob/base64 ở mọi cấp và cấm data URL. Client chỉ giữ telemetry counter/timing tổng hợp trong localStorage, không có sessionId/studentId/ảnh.
6. **Chất lượng/capture**: nút `Chụp & chấm` lấy frame từ độ phân giải sensor tới 2200px thay vì frame preview 1280px; quality metrics chỉ cảnh báo trong phase hiện tại, marker/paper/geometry/confidence vẫn là hard gates.
7. **Gate bật auto batch mặc định**: chưa được coi là đạt độ chính xác thực địa cho tới khi corpus camera đã khử định danh chứng minh: false accept = 0 ở negative set; exact sheet ≥99,5% normal và ≥98% stress; first capture ≥95%; 100% blank/multi/weak/template mismatch được route đúng; p95 detector ≤150ms. Cho tới lúc đó `Chấm Ổn Định` (chọn học sinh → chụp → review → lưu) là CTA chính; QR+OMR tự động là tùy chọn.
