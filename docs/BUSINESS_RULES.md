...
- **Phiên chấm bài (ExamSessionView)**: `chunhiem`/`phuta` chỉ tạo/xem/xóa phiên ở **lớp được phân công** (SSOT `catechist_assignments`). Khi bộ lọc ở chế độ "Tất cả các lớp" (`selectedClassId === null` hoặc `'all'`), hệ thống tự động tải và hiển thị danh sách tất cả các phiên chấm (admin thấy toàn bộ xứ đoàn, GLV thấy các lớp phân công) kèm badge tên lớp trên từng thẻ phiên. Nút "Tạo Phiên Chấm" luôn khả dụng và cung cấp bộ chọn lớp trong form khi đang ở chế độ xem tất cả các lớp. (A-NEW-48 / ADR-025)
