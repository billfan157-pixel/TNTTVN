...
3. **Giới Hạn & Ràng Buộc**:
   ...
   - **UI-POLISH (2026-08-25) — Import theo phần**: form tạo phiên có 2 ô import riêng (Phần Trắc Nghiệm / Phần Tự Luận). `scopeExamParseResult()` lọc kết quả parse theo scope, bỏ phần không thuộc scope kèm warning, đánh lại index 1..N trên phần giữ lại; ghép 2 phần: TN 1..N + TL N+1..N+M (giữ ràng buộc MC contiguous §21.5). Chỉ nạp TL mà chưa có TN → chặn submit với thông báo; chỉ TN → `multiple_choice`; có cả hai → `mixed`.
