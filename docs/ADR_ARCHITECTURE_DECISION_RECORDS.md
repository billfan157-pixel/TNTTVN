...
1. **Owner continuity (S19/OS-02)** — queue ownership is the exact document-local `parishId:userId`, with a captured tenant revision fencing asynchronous work. Legacy rows with unknown parish ownership are quarantined, never assigned to the current account. Individual commands and grade/attendance batches recheck the captured owner after payload preparation and immediately before API dispatch; transport captures continuity before refresh/bootstrap and retains it through retries. Response/ACK reconciliation keeps the same owner (ADR-045).
...
3. **Safe Queue Compaction (OS-01; amended 2026-09-18)** — `CREATE` + `DELETE` pairs are only canceled if `CREATE` is provably unsent local-only (`pending`, `retryCount` 0, no prior error). If `CREATE` may have been sent, keep BOTH `CREATE` and `DELETE` so Phase 1.5 retries the byte-stable original `CREATE`, remaps Temp ID to Real ID, and Phase 3 executes `DELETE` with real ID. The same boundary applies to `CREATE` + `UPDATE`: only a provably unsent CREATE may absorb edits; after possible dispatch, edits compact into a separate UPDATE.
4. **Payload-bound idempotency (OS-01; amended 2026-09-18)** — `idempotencyKey` is passed consistently for Student, Class, Notice and Exam. Each new CREATE commits a SHA-256 fingerprint of its normalized create intent (not a plaintext payload copy) in the existing CREATE audit row, in the same transaction as the entity. Replay compares that immutable fingerprint rather than mutable current fields: same key/same original intent returns the current canonical row even after legitimate UPDATEs; same key/different intent returns `IDEMPOTENCY_CONFLICT` with the already-authorized canonical row for reconciliation. Pre-amendment rows lacking a fingerprint fall back to normalized persisted-row comparison. Legacy student/class/notice queue rows with durable evidence of prior dispatch may convert their formerly folded intent into an owner-bound UPDATE before ACK/remap. Legacy exam rows extract only supported folded `complete|reopen|save_results|remove_result` commands; no generic exam mutation is invented. Fresh key collisions and unsupported mutations fail closed. No schema change is required.
...
8. **Snapshot-checked compaction (OS-04)** — duplicate operations are merged within the exact owner scope. Crypto remains outside the Dexie write transaction; inside that transaction, compaction re-reads and compares the complete pending/retrying owner snapshot, including payload, status and ACK. A changed snapshot aborts the compaction without deleting intent. Grade/notice patch enqueue also compares its encrypted source row before replacing it; disjoint edits are retained.
9. **Conflict edit intent (OS-03)** — Grade queue writes contain only explicitly edited scores/comments and their paired source metadata, marked `_syncGradePatch`. The matrix tracks dirty fields, not just dirty students. Retry uses those fields with the server version, retaining newer pending edits; unedited fields in the local projection come from the server. Legacy full grade snapshots lack sufficient intent evidence: on conflict they remain failed with encrypted diagnostic evidence, not automatically rewritten as manual overrides. Attendance retains its field-level merge; other CRUD retains its existing server-result policy. Enqueue failure keeps the editor draft and rejects success (ADR-109).
10. **Exam acknowledgement barrier** — queued `complete` cannot dispatch while the session has an unsettled `exam_result` command, including failed, processing or unreconciled ACK rows. A known committed complete receipt is reconciled without a blind resend. Server finalization remains authoritative.
11. **Generation preflight** — before claiming or dispatching queued mutations, the coordinator requires a successful purge-generation probe. Missing evidence blocks push and retains pending work. An advanced generation invokes the existing reset contract; a missing local baseline may be adopted only when no owned unsettled commands exist. This is a client preflight, not an atomic server-side generation fence against a purge concurrent with a command.
12. **Daily-entry admission** — add/remove resolves only after durable queue insertion. Preview projection, success feedback and auto-advance follow that acknowledgement; failure retains input. The ledger and server grade policy remain authoritative.
13. **Notice continuity** — direct CREATE and network-fallback CREATE share one intent key. An update with an unsettled same-owner notice command joins the queue instead of bypassing it; overlapping updates in one document serialize through that decision. Failed/in-flight predecessor commands block later notice dispatch. This does not define a global ordering between independent devices.
...
### Amendment 2026-09-21 (Mảng-layer task authority, Field Lead appointment, deputy delegation — product-decided)

- **U-20 / Gói A — tầng Mảng sở hữu task của mình:** trong event `XU_DOAN`, `operations.task.create` và `operations.task.manage` chỉ được cấp khi caller là Trưởng/Phó Ban-Ngành đang hiệu lực của đúng `sourceUnitId` của Mảng (`isUnitLeaderFor`/`isUnitDeputyFor`) hoặc là `WORKSTREAM_LEAD` của chính Mảng đó; mọi nhánh khác bị chốt sớm trong `decideOperationsAuthorization` (`FIELD_TASK_LAYER_CAPABILITIES` → deny `POSITION_SCOPE`, trước cả nhánh role lẫn position). Hệ quả: `EVENT_ORGANIZER`/`EVENT_CREATOR` không còn là đường vòng — Trưởng Xứ đoàn (organizer bắt buộc của event Xứ đoàn), Phó Xứ đoàn và Thư ký không tạo/sửa task bên trong Mảng kể cả với event do họ tạo; họ vẫn view/transition/cancel/publish/comment và vẫn điều phối cấu trúc Mảng (`workstream.create`/`manage`/`mark_ready`). Task không thuộc Mảng (template instantiate, follow-up, dữ liệu grandfather) giữ nguyên hành vi — lỗ hổng đã biết, cố ý để lại đợt sau.
- **Bổ nhiệm Field Lead:** Trưởng Xứ đoàn được cấp `operations.workstream.assign_lead` **chỉ** khi graph là event `XU_DOAN` và Mảng có `sourceUnitId`; target vẫn bị `assertWorkstreamLeadEligibility` + target-scope ép đúng Trưởng Ban/Ngành đương nhiệm, Mảng legacy thiếu `sourceUnitId` không mở đường. Đây là ngoại lệ duy nhất của luật "văn phòng xứ không delegate"; mọi delegate khác của Trưởng Xứ đoàn vẫn bị chặn.
- **Phó Ban/Ngành delegate trong unit:** `UNIT_DEPUTY_CAPABILITIES` thêm `operations.task.assign` + `operations.task.reassign`; target-scope unit/descendants không đổi nên cross-unit vẫn `TARGET_OUTSIDE_ORGANIZATION_SCOPE`; Phó vẫn không có `assign_lead` và không làm Field Lead (O3 giữ nguyên).
- **Permissions projection:** `GET /permissions?eventId=` của event `XU_DOAN` trả `operations.task.create=false` cho caller không có quyền tầng Mảng ở bất kỳ Mảng nào (task chỉ tồn tại bên trong Mảng), và `GET /permissions?workstreamId=` trả đúng theo từng Mảng; map vẫn chỉ là UX, command path tự kiểm capability trên resource thật.
- **Admin override (O2) không đổi.** Bằng chứng: `server/src/services/operationsAuthorization.ts` (`FIELD_TASK_LAYER_CAPABILITIES`, `isUnitLeaderFor`/`isUnitDeputyFor`, chốt tầng Mảng, nhánh `assign_lead` cho `parishLeader`, projection `getOperationsCallerPermissions`) + `server/src/__tests__/operations.test.ts` (field-layer, Gói A creator-role + admin override + permissions map, Mức 3 deputy assign/reassign).
