...
## ADR-110: Task & Event Operations Bounded Context (2026-09-07; amended 2026-09-08)
...
Catevia needs an internal coordination workspace without turning its public parish calendar into a task database or encoding organizational titles as account roles. Operations therefore remains a module of the existing modular monolith and owns `operation_events`, workstreams, tasks, assignments, checklist, participants, comments, reminders and idempotency receipts. `parish_events` remains the only authoritative public calendar source. An operation event may link at most one active same-parish calendar event, and any `PUBLIC_SUMMARY` operation event must have that link; task, assignee, comment and readiness details never enter `/api/parish-events`.

Authorization composes three distinct layers: coarse authenticated account baseline (`users.role`), active organizational position and unit scope (`parish_people` + explicit `parish_service_terms.position_code` + `parish_organization_units`), and resource-specific operational roles. `position_title` is display text and never grants authority. Trưởng Xứ đoàn, Trưởng ngành and Trưởng ban are organizational positions, not new `users.role` values. Event organizer, workstream membership and task assignment are explicit resource authority. An assignment grants execution/approval authority only after `ACCEPTED`: accepted `OWNER|CONTRIBUTOR` may execute, accepted `APPROVER` may approve, and `OBSERVER` may only view/comment. Pending/declined assignments retain only the visibility needed to inspect/respond and never satisfy accepted-owner readiness. An admin can manage the tenant domain but does not become task executor or approver by implication. All decisions are recomputed server-side with tenant/resource predicates; route policy is UX only. Authority-bearing changes to event scope/organizer/public-calendar linkage and workstream scope additionally require organizational create authority at the destination scope.

Every Operations mutation requires `Idempotency-Key`, stores a canonical request hash and response receipt in the same transaction, and rejects key reuse with a different command/payload. Browser CORS preflight explicitly permits both supported idempotency header names. A policy-gated maintenance pass may compact an old receipt response body, but it preserves the immutable parish/actor/key, command and request hash as a tombstone; a late identical retry returns `IDEMPOTENCY_REPLAY_EXPIRED` and never executes again. No retention duration is enabled without approved deployment configuration. Mutable aggregates use version compare-and-swap and audit in the same transaction. Assignment identity is independent of role so a task can have multiple contributors/observers while a partial UNIQUE index enforces at most one active owner. Required workstream/task/checklist readiness is computed on read. Required cancellation does not count as completion, cancellation requires a stored reason with a DB trigger backstop, and a task requiring approval only completes after `APPROVED`; dependency cycles and incomplete required checklist fail closed. Offset timestamps are normalized to UTC before persistence/comparison while event IANA timezone remains separate.

Resource identity is fail-closed: caller-supplied event/workstream/task IDs must form one persisted graph before their roles can be combined. Required readiness counts only leads/accepted owners backed by a current active nondeleted staff account; an unlinked person remains a planning record, not an actionable owner. Parent accounts and parent-linked people cannot be organizer/operational assignee/lead/reminder recipient. READY is revalidated on transition to LIVE. Event configuration is immutable from LIVE; terminal tasks reject later structure/assignment/checklist/dependency/approval changes, while retrospective comments and authorized attendance correction remain separate history/correction paths. Completed/cancelled events cannot acquire new planning children.
...
Migration `20260908-230` adds the controlled `position_code` vocabulary `PARISH_LEADER|BRANCH_LEADER|COMMITTEE_LEADER`. Legacy backfill accepts only exact leader titles in a compatible active unit; titles such as `Phó trưởng ngành` remain unprivileged. Service validation and migrations `231` triggers enforce code/unit compatibility, while the resolver also checks current term dates and active units. Broadening `users.role` remains rejected. Existing legacy workstream leader fields are backfilled into membership rows for compatibility and are no longer authorization sources.
