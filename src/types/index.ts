...
export interface ExamQuestion {
  ...
  points?: number;
  /** Question Bank provenance copied into the immutable exam snapshot. */
  sourceQuestionId?: string;
  sourceVersionId?: string;
}

export interface ExamVariantManifestEntry {
  version: ExamVersionCode;
  sourceQuestionOrder: number[];
  sourceQuestionIds: string[];
  optionOrders: Record<number, MultipleChoiceOption[]>;
...
export interface ExamVariantManifestSet {
  ...
  sourceHash: string;
  generatedAt: string;
  variants: Partial<Record<ExamVersionCode, ExamVariantManifestEntry>>;
}
...
export interface ExamSession {
  ...
  /** Materialized, immutable question/option permutations generated by the server. */
  variantManifests?: ExamVariantManifestSet | string;
  questions?: ExamQuestion[] | string;
  sourceType?: 'legacy' | 'question_bank' | 'blueprint';
  blueprintId?: string | null;
  blueprintSnapshot?: string | Record<string, unknown> | null;
}

export type QuestionBankType = 'multiple_choice' | 'true_false' | 'multiple_select' | 'short_answer' | 'fill_blank' | 'matching' | 'essay';
export type QuestionBankStatus = 'draft' | 'in_review' | 'approved' | 'active' | 'archived';
...
export interface QuestionBankItem {
  ...
  createdBy: string;
  updatedAt: string;
  current: QuestionBankVersion;
  versions?: QuestionBankVersion[];
  usage?: Array<{ examSessionId: string; position: number; subject: string; classId: string; academicYear: string; usedAt: string }>;
}
