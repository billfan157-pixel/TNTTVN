...
it('saves structured lessons and creates one atomic follow-up with an actionable owner', async () => {
  ...
  await waitFor(() => expect(operationsApi.saveEventRetrospective).toHaveBeenCalledWith('event-1', { expectedVersion: null, lessonsLearned: 'Phân công sớm.', improvementNotes: 'Chốt vật dụng trước ba ngày.' }))
  ...
  await waitFor(() => expect(operationsApi.createEventFollowUp).toHaveBeenCalledWith('event-1', expect.objectContaining({ eventVersion: 5, title: 'Chuẩn hóa checklist', personId: 'person-1', priority: 'HIGH' })))
