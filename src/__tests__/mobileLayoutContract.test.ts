...
describe('App-wide mobile layout contract', () => {
  ...
  it('defines one mobile safe-area and touch-target contract for shells and portaled dialogs', () => {
    ...
    expect(css).toMatch(/\.mobile-control-field select,\s*\.mobile-control-search\s*\{\s*min-height: 44px;/)
    expect(css).toMatch(/\.mobile-control-field select,\s*\.mobile-control-search\s*\{[\s\S]*?font-size: 16px;/)
