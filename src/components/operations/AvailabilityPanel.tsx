...
export function AvailabilityPanel({ enabled }: { enabled: boolean }) {
  ...
  const mutate = async (action: () => Promise<unknown>, success: string, actionKey = 'general') => {
    const scope = getTenantScope()
    if (!enabled || !scope || inFlight.current) return false
    inFlight.current = true; setBusyAction(actionKey); setMessage('')
    ...
    try {
      ...
      return false
    } finally { inFlight.current = false; if (sameSession()) setBusyAction(null) }
  }
  ...
  return <section className="space-y-3 rounded-xl border border-surface-border p-4" aria-label="Lịch bận của tôi">
    ...
    <form className="grid gap-2 sm:grid-cols-[1fr_1fr_1fr_auto] sm:items-end" onSubmit={event => {
      ...
      void mutate(async () => {
        ...
        return result
      }, 'Đã lưu lịch bận.', 'create').then(saved => {
        if (saved && alive.current) setDraft({ startsAt: '', endsAt: '', reason: '' })
      })
    }}>
      <TextInput aria-label="Bận từ" type="datetime-local" value={draft.startsAt} disabled={!enabled || isBusy('create')} required onChange={event => setDraft(value => ({ ...value, startsAt: event.target.value }))} />
      <TextInput aria-label="Bận đến" type="datetime-local" value={draft.endsAt} disabled={!enabled || isBusy('create')} required onChange={event => setDraft(value => ({ ...value, endsAt: event.target.value }))} />
      <TextInput aria-label="Lý do bận riêng tư" value={draft.reason} maxLength={500} disabled={!enabled || isBusy('create')} placeholder="Lý do (không bắt buộc)" onChange={event => setDraft(value => ({ ...value, reason: event.target.value }))} />
      <Button type="submit" size="sm" disabled={!enabled || isBusy('create') || !validWindow(draft.startsAt, draft.endsAt)}>Báo bận</Button>
    </form>
    ...
    <div className="divide-y divide-surface-border">
      {rows.map(row => <div key={row.id} data-blockout-id={row.id} className="py-3 text-sm text-text-main">
        <div className="flex flex-wrap items-center justify-between gap-2"><span>{new Date(row.startsAt).toLocaleString('vi-VN')} – {new Date(row.endsAt).toLocaleString('vi-VN')}{row.reason ? ` · ${row.reason}` : ''}</span><div className="flex gap-2"><Button variant="secondary" size="sm" disabled={!enabled || isBusy(`revoke:${row.id}`) || isBusy(`update:${row.id}`)} onClick={() => setEditing({ id: row.id, startsAt: localDateTime(row.startsAt), endsAt: localDateTime(row.endsAt), reason: row.reason ?? '' })}>Sửa</Button><Button variant="danger" size="sm" disabled={!enabled || isBusy(`revoke:${row.id}`) || isBusy(`update:${row.id}`)} onClick={() => {
          ...
          return void mutate(async () => {
            ...
            return result
          }, 'Đã thu hồi lịch bận.', `revoke:${row.id}`)
        ...
        {editing?.id === row.id && <div className="mt-2 grid gap-2 sm:grid-cols-[1fr_1fr_1fr_auto] sm:items-end">
          ...
          <TextInput aria-label={`Sửa bận từ của khoảng ${blockoutRowLabel(row)}`} type="datetime-local" value={editing.startsAt} disabled={isBusy(`update:${row.id}`)} onChange={event => setEditing(value => value ? { ...value, startsAt: event.target.value } : value)} />
          <TextInput aria-label={`Sửa bận đến của khoảng ${blockoutRowLabel(row)}`} type="datetime-local" value={editing.endsAt} disabled={isBusy(`update:${row.id}`)} onChange={event => setEditing(value => value ? { ...value, endsAt: event.target.value } : value)} />
          <TextInput aria-label={`Sửa lý do của khoảng ${blockoutRowLabel(row)}`} value={editing.reason} maxLength={500} disabled={isBusy(`update:${row.id}`)} onChange={event => setEditing(value => value ? { ...value, reason: event.target.value } : value)} />
          <Button size="sm" disabled={isBusy(`update:${row.id}`) || !validWindow(editing.startsAt, editing.endsAt)} onClick={() => {
            ...
            return void mutate(async () => {
              ...
              return result
            }, 'Đã cập nhật lịch bận.', `update:${row.id}`)
          }}>Lưu</Button>
        </div>}
      </div>)}
    </div>
