    ...
    {snapshot && (
      ...
      <div role="dialog" aria-modal="true" aria-label="Ảnh rà soát phiếu" className="app-modal-layer--nested fixed inset-0 flex items-center justify-center bg-black/80 p-3" onClick={closeSnapshot}>
        <div ref={trapRef} className="flex max-h-[94vh] max-w-3xl flex-col overflow-hidden rounded-2xl bg-surface-card p-3" onClick={event => event.stopPropagation()}>
          <div className="mb-2 flex items-center justify-between gap-3"><span className="text-xs font-semibold text-text-muted">Ảnh cục bộ · tự xóa lúc {new Date(snapshot.expiresAt).toLocaleString('vi-VN')}</span><button type="button" className="btn btn-secondary btn-sm" onClick={closeSnapshot}><X size={14} /> Đóng</button></div>
